...
function rep(file, old, novo, tag) {
  ...
  if (c.includes(old)) { c = c.replace(old, novo); fs.writeFileSync(file,'utf8'); console.log('OK: '+tag); ok++; }
...
// ─────────────────────────────────────────────────────────────────────────────
{
  let c = fs.readFileSync('server/db.ts').toString('utf8').replace(/\r\n/g,'\n');
  const insertPoint = '// ─── Vendas ───────────────────────────────────────────────────────────────────';
  if (!c.includes('getBranches') && c.includes(insertPoint)) {
    const branchCode = `
// ─── Filiais (Plano Estúdio) ──────────────────────────────────────────────────
...
`;
    c = c.replace(insertPoint, branchCode + insertPoint);
    fs.writeFileSync('server/db.ts', c, 'utf8');
    console.log('OK: db.ts funções filiais'); ok++;
  } else {
    console.log(c.includes('getBranches') ? 'SKIP: db.ts já tem getBranches' : 'MISS: db.ts ponto de inserção');
  }
}
...
// ─────────────────────────────────────────────────────────────────────────────
{
  ...
  // 3a. Botão Desativar → Ocultar do agendamento
  const oldBtn = `\${s.isActive ? "Desativar" : "Ativar"}</button>`;
  const newBtn = `\${s.isActive ? "Ocultar do agendamento" : "Exibir no agendamento"}</button>`;
  if (c.includes(oldBtn)) { c = c.replace(oldBtn, newBtn); console.log('OK: label botão serviços'); ok++; }
  else console.log('MISS: botão serviços');

  // 3b. Lookup de filiais em renderConfiguracoes
  const oldTenant = `  const tenant = barber?.tenantId ? await db.getTenantById(barber.tenantId) : undefined;\n  const currentSlug = tenant?.slug ?? "";`;
  const newTenant = `  const tenant = barber?.tenantId ? await db.getTenantById(barber.tenantId) : undefined;\n  const parentTenantId = (tenant as any)?.parentTenantId ?? null;\n  const isStudioPlan = ((tenant as any)?.plan ?? '') === 'studio';\n  let branches: any[] = [];\n  if (isStudioPlan && !parentTenantId && barber?.tenantId) branches = await db.getBranches(barber.tenantId);\n  const showBranchTab = isStudioPlan && !parentTenantId;\n  const currentSlug = tenant?.slug ?? "";`;
  if (c.includes(oldTenant)) { c = c.replace(oldTenant, newTenant); console.log('OK: branches lookup'); ok++; }
  else console.log('MISS: tenant lookup');
  ...
  if (c.includes(oldTabs)) { c = c.replace(oldTabs, newTabs); console.log('OK: tabs com filiais'); ok++; }
  else console.log('MISS: tabs');

  // 3d. tabFiliais no tabContent
  const oldTC = `    equipe: tabEquipe,\n    pagamentos: tabPagamentos,\n  };`;
  const newTC = `    equipe: tabEquipe,\n    filiais: tabFiliais,\n    pagamentos: tabPagamentos,\n  };`;
  if (c.includes(oldTC)) { c = c.replace(oldTC, newTC); console.log('OK: tabFiliais no tabContent'); ok++; }
  else console.log('MISS: tabContent');

  // 3e. renderBranchTab + rotas CRUD filiais
  const routeGET = `  app.get("/admin/configuracoes", requireAdminAuth, async (req: Request, res: Response, next: NextFunction) => {`;
  const branchInjection = `  // ── renderBranchTab ────────────────────────────────────────────────────────
  ...
  if (c.includes(routeGET)) { c = c.replace(routeGET, branchInjection); console.log('OK: rotas filiais + renderBranchTab'); ok++; }
  else console.log('MISS: routeGET configuracoes');
