...
export default function DashboardScreen() {
  ...
  const monthlySalesQuery = trpc.sales.byDateRange.useQuery(
    ...
    { enabled: !!barber?.tenantId, refetchOnMount: true }
  );
  ...
  const trialDaysLeft = tenant?.trialEndsAt
    ? (() => {
        // Compara datas no fuso local (Brasil) ignorando hora,
        // evitando falsos "expira hoje" por diferença de UTC.
        const endDate = new Date(tenant.trialEndsAt);
        const todayLocal = new Date();
        // Zeramos a hora de ambas para comparar só o dia
        endDate.setHours(0, 0, 0, 0);
        todayLocal.setHours(0, 0, 0, 0);
        const diffMs = endDate.getTime() - todayLocal.getTime();
