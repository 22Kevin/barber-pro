...
async function renderAgenda(req: Request, res: Response) {
  ...
  try {
    const dbConn = await db.getDb();
    if (dbConn && tenantId) {
      const rawP = await dbConn.execute(sql`SELECT id, name, price, recurrences, "maxServices", "maxProducts" FROM subscription_plans WHERE "tenantId" = ${tenantId} AND "isActive" = true ORDER BY name`) as any;
      agendaPlans = Array.isArray(rawP) ? (rawP[0] as any[]) : (rawP?.rows ?? []);
      // Buscar serviços e produtos de cada plano
      for (const plan of agendaPlans) {
        try {
          const svcs = await dbConn.execute(sql`SELECT sps."serviceId", s.name as "serviceName", s."durationMinutes" FROM subscription_plan_services sps JOIN services s ON s.id = sps."serviceId" WHERE sps."planId" = ${plan.id}`) as any;
          plan.services = Array.isArray(svcs) ? (svcs[0] as any[]) : (svcs?.rows ?? []);
          const prds = await dbConn.execute(sql`SELECT spp."productId", p.name as "productName" FROM subscription_plan_products spp JOIN products p ON p.id = spp."productId" WHERE spp."planId" = ${plan.id}`) as any;
          plan.products = Array.isArray(prds) ? (prds[0] as any[]) : (prds?.rows ?? []);
        } catch(e) { plan.services = []; plan.products = []; }
      }
    }
  ...
  const agendaPlansByIdJson = JSON.stringify(
    agendaPlans.reduce((acc: any, p: any) => {
      acc[p.id] = {
        id: p.id, name: p.name, price: p.price, recurrences: p.recurrences,
        maxServices: p.maxServices ?? 999, maxProducts: p.maxProducts ?? 999,
...
async function renderFinanceiro(req: Request, res: Response) {
  ...
  const pmLabels: Record<string, string> = { cash: "Dinheiro", credit_card: "Cartão Crédito", debit_card: "Cartão Débito", pix: "Pix", asaas: "Online (Asaas)", other: "Outro" };
...
<body style="margin:0;padding:0;background:#0A0A0A;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif">
  ...
  app.get("/admin/suporte", requireAdminAuth, withErrorPage("Suporte", "suporte", async (req: Request, res: Response) => {
    ...
    const tutorials = [
      {
        id: "agenda", icon: "📅", title: "Como criar um agendamento", color: "#C9A84C",
        steps: [
          { title: "Acesse a Agenda", desc: "Clique em <strong>Agenda</strong> no menu lateral esquerdo. Você verá o calendário do dia com todos os horários.", mockup: "agenda" },
          { title: "Novo Agendamento", desc: "Clique no botão <strong>+ Novo Agendamento</strong> no canto superior direito da tela.", mockup: "novo-appt" },
          { title: "Preencha os dados", desc: "Selecione o <strong>cliente</strong>, o <strong>serviço</strong>, o <strong>barbeiro</strong> e o <strong>horário</strong> desejado.", mockup: "form-appt" },
          { title: "Confirme!", desc: "Clique em <strong>Confirmar Agendamento</strong>. O cliente recebe um <strong>WhatsApp automático</strong> com os detalhes.", mockup: "confirm-appt" },
        ]
      },
      {
        id: "pagamento", icon: "💰", title: "Como registrar um pagamento", color: "#4ADE80",
        steps: [
          { title: "Selecione o agendamento", desc: "Na Agenda, localize o agendamento com status <strong>Em Andamento</strong> (azul). Clique nele.", mockup: "appt-inprogress" },
          { title: "Clique em Concluir", desc: "No painel do agendamento, clique no botão <strong>Concluir</strong>. Um modal de pagamento aparecerá.", mockup: "btn-concluir" },
          { title: "Escolha a forma de pagamento", desc: "Selecione entre <strong>Dinheiro</strong>, <strong>Pix</strong> (gera QR Code) ou <strong>Cartão</strong>.", mockup: "payment-modal" },
          { title: "Venda registrada!", desc: "A venda é criada <strong>automaticamente no Financeiro</strong>. Um toast confirma o pagamento.", mockup: "payment-done" },
        ]
      },
      {
        id: "cliente", icon: "👤", title: "Como cadastrar um cliente", color: "#60A5FA",
        steps: [
          { title: "Acesse Clientes", desc: "Clique em <strong>Clientes</strong> no menu lateral. Você verá a lista de todos os clientes cadastrados.", mockup: "clientes-list" },
          { title: "Novo Cliente", desc: "Clique no botão <strong>+ Novo Cliente</strong> no topo da página.", mockup: "novo-cliente" },
          { title: "Preencha os dados", desc: "Digite o <strong>nome</strong> e o <strong>telefone</strong> (obrigatórios). E-mail, data de nascimento e notas são opcionais.", mockup: "form-cliente" },
          { title: "Cliente salvo!", desc: "Pronto! O cliente já aparece na lista e pode <strong>agendar pela página pública</strong>.", mockup: "cliente-salvo" },
        ]
      },
      {
        id: "servico", icon: "✂️", title: "Como adicionar um serviço", color: "#A78BFA",
        steps: [
          { title: "Acesse Serviços", desc: "Clique em <strong>Serviços</strong> no grupo Catálogo do menu lateral.", mockup: "servicos-list" },
          { title: "Novo Serviço", desc: "Clique em <strong>+ Novo Serviço</strong>.", mockup: "novo-servico" },
          { title: "Configure o serviço", desc: "Defina o <strong>nome</strong>, a <strong>duração</strong> em minutos e o <strong>preço</strong>. Adicione uma foto para destacar.", mockup: "form-servico" },
          { title: "Publicado!", desc: "O serviço aparece automaticamente na <strong>página de agendamentos</strong> dos seus clientes.", mockup: "servico-pub" },
        ]
      },
      {
        id: "financeiro", icon: "📊", title: "Como usar o Financeiro", color: "#FB923C",
        steps: [
          { title: "Acesse o Financeiro", desc: "Clique em <strong>Financeiro</strong> no grupo Financeiro do menu.", mockup: "financeiro-dash" },
          { title: "Visão geral", desc: "Veja <strong>receitas</strong>, <strong>despesas</strong> e <strong>saldo do período</strong> nos cards do topo.", mockup: "financeiro-cards" },
          { title: "Filtre por período", desc: "Use os botões de data para ver <strong>hoje</strong>, <strong>esta semana</strong>, <strong>este mês</strong> ou um período personalizado.", mockup: "financeiro-filtros" },
          { title: "Exporte relatórios", desc: "Clique em <strong>Relatórios</strong> no menu para exportar em <strong>PDF</strong> ou <strong>CSV</strong>.", mockup: "financeiro-export" },
        ]
      },
      {
        id: "pagina", icon: "🌐", title: "Como personalizar sua página pública", color: "#34D399",
        steps: [
          { title: "Acesse Página do Cliente", desc: "Clique em <strong>Página do Cliente</strong> no menu. Você verá um preview da sua página.", mockup: "pagina-preview" },
          { title: "Personalize a aparência", desc: "Altere a <strong>cor principal</strong>, faça upload do <strong>logo</strong> e da <strong>foto de capa</strong>.", mockup: "pagina-config" },
          { title: "Configure horários", desc: "Defina os <strong>dias e horários</strong> de funcionamento. Clientes só conseguem agendar nesses horários.", mockup: "pagina-horarios" },
          { title: "Compartilhe o link!", desc: "Copie seu link único e cole no <strong>Instagram</strong>, <strong>WhatsApp</strong> e onde quiser.", mockup: "pagina-link" },
        ]
      },
      {
        id: "estoque", icon: "📦", title: "Como gerenciar estoque e produtos", color: "#FBBF24",
        steps: [
          { title: "Acesse Estoque", desc: "Clique em <strong>Estoque</strong> no grupo Catálogo do menu. Veja todos os produtos com quantidade atual e mínima.", mockup: "estoque-list" },
          { title: "Registre uma entrada", desc: "Clique em <strong>Entrada</strong> no produto desejado. Informe a quantidade recebida e o fornecedor.", mockup: "estoque-entrada" },
          { title: "Alertas de estoque baixo", desc: "Quando a quantidade cair abaixo do mínimo, um <strong>alerta amarelo</strong> aparece no card. Configure o mínimo em Produtos.", mockup: "estoque-alerta" },
          { title: "Histórico de movimentações", desc: "Clique em <strong>Histórico</strong> para ver todas as entradas e saídas do produto com data e responsável.", mockup: "estoque-historico" },
        ]
      },
      {
        id: "assinaturas", icon: "🌟", title: "Como criar planos de assinatura", color: "#A78BFA",
        steps: [
          { title: "Acesse Planos de Assinatura", desc: "Clique em <strong>Planos de Assinatura</strong> no menu. Veja os planos criados e quantos clientes assinam cada um.", mockup: "planos-list" },
          { title: "Crie um novo plano", desc: "Clique em <strong>+ Novo Plano</strong>. Defina nome, preço mensal e descrição atrativa.", mockup: "plano-novo" },
          { title: "Adicione serviços e produtos", desc: "Selecione quais <strong>serviços</strong> estão incluídos (ex: 2 cortes/mês) e quais <strong>produtos</strong> o assinante recebe.", mockup: "plano-servicos" },
          { title: "Clientes assinam pela página", desc: "Compartilhe o link da sua página. Na aba <strong>Assinaturas</strong>, clientes escolhem o plano e assinam online.", mockup: "plano-pub" },
        ]
      },
      {
        id: "fidelidade", icon: "⭐", title: "Como usar o programa de fidelidade", color: "#F59E0B",
        steps: [
          { title: "Acesse Fidelidade", desc: "Clique em <strong>Fidelidade</strong> no menu. Configure o programa de pontos da sua barbearia.", mockup: "fidelidade-config" },
          { title: "Configure as regras", desc: "Defina quantos <strong>pontos por real</strong> gasto e quais <strong>recompensas</strong> os clientes podem resgatar.", mockup: "fidelidade-regras" },
          { title: "Clientes acumulam pontos", desc: "A cada venda registrada, os pontos são adicionados automaticamente ao perfil do cliente.", mockup: "fidelidade-pontos" },
          { title: "Resgate de recompensas", desc: "No perfil do cliente, clique em <strong>Resgatar</strong> para trocar os pontos por serviços ou descontos.", mockup: "fidelidade-resgate" },
        ]
      },
      {
        id: "avaliacoes", icon: "⭐", title: "Como gerenciar avaliações", color: "#EF4444",
        steps: [
          { title: "Acesse Avaliações", desc: "Clique em <strong>Avaliações</strong> no menu. Veja todas as notas e comentários dos clientes.", mockup: "avaliacoes-list" },
          { title: "Pedido automático pós-atendimento", desc: "Após concluir um agendamento, o sistema <strong>envia automaticamente</strong> um WhatsApp e e-mail pedindo avaliação.", mockup: "avaliacoes-auto" },
          { title: "Responda as avaliações", desc: "Clique em uma avaliação para <strong>responder publicamente</strong>. Sua resposta aparece na página pública.", mockup: "avaliacoes-resposta" },
          { title: "Nota média na página pública", desc: "A média das avaliações aparece automaticamente na <strong>página pública</strong> da sua barbearia.", mockup: "avaliacoes-pub" },
        ]
      },
      {
        id: "comissoes", icon: "💸", title: "Como configurar comissões", color: "#10B981",
        steps: [
          { title: "Acesse Comissões", desc: "Clique em <strong>Comissões</strong> no menu. Configure o percentual de cada barbeiro.", mockup: "comissoes-config" },
          { title: "Configure por barbeiro", desc: "Defina um <strong>percentual padrão</strong> para cada barbeiro ou configure por serviço individualmente.", mockup: "comissoes-barbeiro" },
          { title: "Comissões calculadas automaticamente", desc: "A cada venda registrada, a comissão é <strong>calculada e acumulada</strong> automaticamente.", mockup: "comissoes-calc" },
          { title: "Barbeiro vê suas comissões", desc: "Em <strong>Minhas Comissões</strong>, cada barbeiro vê o total acumulado no período.", mockup: "comissoes-view" },
        ]
      },
      {
        id: "configuracoes", icon: "⚙️", title: "Como configurar o sistema", color: "#6B7280",
        steps: [
          { title: "Acesse Configurações", desc: "Clique em <strong>Configurações</strong> no menu. Veja as seções: Dados, Horários, Equipe e Pagamentos.", mockup: "config-menu" },
          { title: "Dados da barbearia", desc: "Atualize <strong>nome</strong>, <strong>endereço</strong>, <strong>telefone</strong> e <strong>redes sociais</strong> da barbearia.", mockup: "config-dados" },
          { title: "Adicione barbeiros", desc: "Na aba <strong>Equipe</strong>, clique em <strong>+ Adicionar Barbeiro</strong>. Defina nome, e-mail e senha de acesso.", mockup: "config-equipe" },
          { title: "Ative pagamentos online", desc: "Na aba <strong>Pagamentos</strong>, conecte sua conta Asaas para receber Pix e cartão diretamente.", mockup: "config-pagamentos" },
        ]
      },
      {
        id: "app", icon: "📱", title: "Como usar o app móvel", color: "#3B82F6",
        steps: [
          { title: "Baixe o app", desc: "Procure por <strong>Barber Pro</strong> na <strong>App Store</strong> (iPhone) ou <strong>Play Store</strong> (Android) e instale gratuitamente.", mockup: "app-download" },
          { title: "Faça login", desc: "Use o <strong>mesmo e-mail e senha</strong> do painel web. Toque em <strong>Entrar</strong>.", mockup: "app-login" },
          { title: "Gerencie pelo celular", desc: "Veja a <strong>agenda do dia</strong>, confirme agendamentos, registre pagamentos e consulte o financeiro.", mockup: "app-agenda" },
          { title: "Ative as notificações", desc: "Permita notificações para receber alertas de <strong>novos agendamentos</strong> em tempo real, sem precisar abrir o app.", mockup: "app-notif" },
        ]
      },
      {
        id: "assinaturas", icon: "🌟", title: "Como criar planos de assinatura", color: "#F59E0B",
        steps: [
          { title: "Acesse Planos de Assinatura", desc: "Clique em <strong>Planos de Assinatura</strong> no menu. Veja todos os planos criados.", mockup: "planos-list" },
          { title: "Crie um novo plano", desc: "Clique em <strong>+ Novo Plano</strong>. Defina nome, preço mensal e uma descrição atrativa.", mockup: "plano-novo" },
          { title: "Adicione serviços incluídos", desc: "Selecione quais <strong>serviços</strong> fazem parte do plano e quantas vezes por mês o cliente pode usar.", mockup: "plano-servicos" },
          { title: "Publique e compartilhe", desc: "O plano aparece na <strong>aba Assinaturas</strong> da sua página pública. Clientes assinam direto pelo app.", mockup: "plano-pub" },
        ]
      },
      {
        id: "produtos", icon: "🧴", title: "Como gerenciar produtos", color: "#EC4899",
        steps: [
          { title: "Acesse Produtos", desc: "Clique em <strong>Produtos</strong> no grupo Catálogo do menu. Veja todos os produtos cadastrados.", mockup: "produtos-list" },
          { title: "Cadastre um produto", desc: "Clique em <strong>+ Novo Produto</strong>. Defina nome, preço, quantidade em estoque e foto.", mockup: "produto-novo" },
          { title: "Controle o estoque", desc: "Acesse <strong>Estoque</strong> para ver alertas de produtos abaixo do mínimo e registrar entradas.", mockup: "estoque-dash" },
          { title: "Venda na página pública", desc: "Produtos aparecem na <strong>aba Produtos</strong> da sua página. Clientes adicionam ao carrinho e pagam online.", mockup: "produto-pub" },
        ]
      },
      {
        id: "relatorios", icon: "📈", title: "Como usar Relatórios e Comissões", color: "#06B6D4",
        steps: [
          { title: "Acesse Relatórios", desc: "Clique em <strong>Relatórios</strong> no grupo Financeiro. Escolha o período desejado.", mockup: "relatorios-dash" },
          { title: "Analise os dados", desc: "Veja <strong>serviços mais vendidos</strong>, <strong>clientes mais frequentes</strong> e <strong>receita por barbeiro</strong>.", mockup: "relatorios-charts" },
          { title: "Configure comissões", desc: "Em <strong>Comissões</strong>, defina o percentual de cada barbeiro por serviço ou valor fixo.", mockup: "comissoes-config" },
          { title: "Exporte os dados", desc: "Clique em <strong>Exportar PDF</strong> ou <strong>CSV</strong> para compartilhar com o seu contador.", mockup: "financeiro-export" },
        ]
      },
      {
        id: "fidelidade", icon: "⭐", title: "Como usar Fidelidade e Avaliações", color: "#EAB308",
        steps: [
          { title: "Configure o programa", desc: "Em <strong>Fidelidade</strong>, defina quantos pontos cada serviço gera e quais recompensas estão disponíveis.", mockup: "fidelidade-config" },
          { title: "Clientes acumulam pontos", desc: "A cada atendimento concluído, o cliente ganha pontos automaticamente. Veja o saldo em <strong>Clientes → Perfil</strong>.", mockup: "fidelidade-pontos" },
          { title: "Gerencie avaliações", desc: "Em <strong>Avaliações</strong>, veja todas as notas e comentários. Responda públicamente para engajar.", mockup: "avaliacoes-dash" },
          { title: "Peça avaliações", desc: "Após concluir um atendimento, um <strong>link de avaliação</strong> é enviado por WhatsApp automaticamente.", mockup: "avaliacoes-whats" },
        ]
      },
      {
        id: "marketing", icon: "📢", title: "Como usar Retorno Automático e Promoções", color: "#8B5CF6",
        steps: [
          { title: "Configure o Retorno Automático", desc: "Em <strong>Retorno Automático</strong>, defina após quantos dias sem visita enviar mensagem de reativação via WhatsApp.", mockup: "retorno-config" },
          { title: "Crie uma promoção", desc: "Em <strong>Promoções</strong>, crie cupons de desconto com valor fixo ou percentual e defina o período de validade.", mockup: "promo-nova" },
          { title: "Distribua os cupons", desc: "Compartilhe o <strong>código do cupom</strong> no Instagram, WhatsApp ou e-mail. Clientes aplicam no checkout.", mockup: "promo-cupom" },
          { title: "Acompanhe resultados", desc: "Em <strong>Conversão de Promoções</strong>, veja quantos cupons foram usados e o valor gerado.", mockup: "promo-resultado" },
        ]
      },
      {
        id: "configuracoes", icon: "⚙️", title: "Como configurar o sistema", color: "#6B7280",
        steps: [
          { title: "Acesse Configurações", desc: "Clique em <strong>Configurações</strong> no menu. É aqui que você personaliza todo o comportamento do sistema.", mockup: "config-geral" },
          { title: "Dados da barbearia", desc: "Atualize <strong>nome</strong>, <strong>endereço</strong>, <strong>telefone</strong> e <strong>redes sociais</strong> da sua barbearia.", mockup: "config-dados" },
          { title: "Ative pagamentos online", desc: "Em <strong>Pagamentos Online</strong>, conecte sua conta Asaas para receber Pix e cartão direto na sua conta.", mockup: "config-asaas" },
          { title: "Notificações e WhatsApp", desc: "Configure o <strong>número do WhatsApp</strong> para lembretes automáticos e defina as mensagens de retorno.", mockup: "config-whats" },
        ]
      },
      {
        id: "app", icon: "📱", title: "Como usar o App Móvel", color: "#10B981",
        steps: [
          { title: "Baixe o aplicativo", desc: "Procure por <strong>Barber Pro</strong> na <strong>App Store</strong> (iOS) ou <strong>Play Store</strong> (Android) e instale.", mockup: "app-download" },
          { title: "Faça login", desc: "Use o mesmo <strong>e-mail e senha</strong> do painel web. O app sincroniza automaticamente.", mockup: "app-login" },
          { title: "Gerencie pelo celular", desc: "Visualize a <strong>agenda do dia</strong>, cadastre clientes, veja o financeiro e responda notificações.", mockup: "app-home" },
          { title: "Ative notificações", desc: "Permita notificações push para ser avisado de <strong>novos agendamentos</strong>, cancelamentos e mensagens.", mockup: "app-notif" },
        ]
      },
    ];

    // Mockups SVG inline para cada passo
    const mockups: Record<string, string> = {
      "agenda": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A"/><rect x="8" y="12" width="40" height="6" rx="3" fill="#C9A84C" opacity=".8"/><rect x="8" y="28" width="40" height="4" rx="2" fill="#333"/><rect x="8" y="38" width="40" height="4" rx="2" fill="#C9A84C" opacity=".6"/><rect x="8" y="48" width="40" height="4" rx="2" fill="#333"/><rect x="8" y="58" width="40" height="4" rx="2" fill="#333"/><rect x="64" y="12" width="100" height="7" rx="3" fill="#EEE"/><rect x="180" y="10" width="70" height="11" rx="5" fill="#C9A84C"/><rect x="64" y="32" width="186" height="1" fill="#222"/><rect x="64" y="40" width="30" height="5" rx="2" fill="#555"/><rect x="104" y="40" width="30" height="5" rx="2" fill="#555"/><rect x="144" y="40" width="30" height="5" rx="2" fill="#555"/><rect x="64" y="55" width="186" height="28" rx="6" fill="#1A1A1A" stroke="#C9A84C" stroke-width="1"/><rect x="70" y="62" width="50" height="4" rx="2" fill="#EEE"/><rect x="70" y="70" width="70" height="3" rx="1" fill="#555"/><rect x="64" y="90" width="186" height="28" rx="6" fill="#1A1A1A" stroke="#333" stroke-width="1"/><rect x="70" y="97" width="50" height="4" rx="2" fill="#EEE"/><rect x="70" y="105" width="60" height="3" rx="1" fill="#555"/><rect x="64" y="125" width="186" height="28" rx="6" fill="#1A1A1A" stroke="#333" stroke-width="1"/><rect x="70" y="132" width="50" height="4" rx="2" fill="#EEE"/><rect x="70" y="140" width="70" height="3" rx="1" fill="#555"/></svg>`,

      "novo-appt": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="12" width="100" height="7" rx="3" fill="#EEE"/><rect x="180" y="10" width="70" height="11" rx="5" fill="#C9A84C"/><text x="195" y="19" fill="#0A0A0A" font-size="7" font-weight="bold" font-family="sans-serif">+ Novo</text><rect x="64" y="40" width="186" height="100" rx="10" fill="#1a1a1a" stroke="#C9A84C" stroke-width="1.5"/><text x="112" y="58" fill="#C9A84C" font-size="9" font-weight="bold" font-family="sans-serif">Novo Agendamento</text><rect x="80" y="66" width="150" height="14" rx="4" fill="#222" stroke="#333" stroke-width="1"/><text x="88" y="76" fill="#666" font-size="7" font-family="sans-serif">Selecionar cliente...</text><rect x="80" y="86" width="150" height="14" rx="4" fill="#222" stroke="#333" stroke-width="1"/><text x="88" y="96" fill="#666" font-size="7" font-family="sans-serif">Selecionar serviço...</text><rect x="80" y="106" width="150" height="14" rx="4" fill="#222" stroke="#333" stroke-width="1"/><text x="88" y="116" fill="#666" font-size="7" font-family="sans-serif">Selecionar horário...</text><rect x="80" y="126" width="150" height="10" rx="4" fill="#C9A84C"/><text x="128" y="133" fill="#0A0A0A" font-size="7" font-weight="bold" font-family="sans-serif">Confirmar</text></svg>`,

      "form-appt": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="60" y="10" width="160" height="140" rx="10" fill="#1a1a1a" stroke="#2a2a2a" stroke-width="1"/><text x="100" y="28" fill="#EEE" font-size="9" font-weight="bold" font-family="sans-serif">Novo Agendamento</text><rect x="72" y="36" width="136" height="14" rx="4" fill="#222" stroke="#C9A84C" stroke-width="1"/><text x="80" y="46" fill="#EEE" font-size="7" font-family="sans-serif">João Silva</text><rect x="72" y="56" width="136" height="14" rx="4" fill="#222" stroke="#333" stroke-width="1"/><text x="80" y="66" fill="#EEE" font-size="7" font-family="sans-serif">Corte + Barba — R$ 55</text><rect x="72" y="76" width="64" height="14" rx="4" fill="#222" stroke="#333" stroke-width="1"/><text x="80" y="86" fill="#EEE" font-size="7" font-family="sans-serif">Carlos</text><rect x="144" y="76" width="64" height="14" rx="4" fill="#222" stroke="#C9A84C" stroke-width="1"/><text x="152" y="86" fill="#C9A84C" font-size="7" font-family="sans-serif">14:00 - 15:00</text><rect x="72" y="100" width="136" height="30" rx="6" fill="#0d2818" stroke="#4ADE8044" stroke-width="1"/><text x="100" y="114" fill="#4ADE80" font-size="8" font-family="sans-serif">✓ Horário disponível</text><rect x="90" y="138" width="100" height="10" rx="4" fill="#C9A84C"/><text x="121" y="145" fill="#0A0A0A" font-size="7" font-weight="bold" font-family="sans-serif">Confirmar</text></svg>`,

      "confirm-appt": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="40" width="186" height="70" rx="8" fill="#1a1a1a" stroke="#333"/><rect x="70" y="47" width="60" height="5" rx="2" fill="#EEE"/><rect x="70" y="57" width="80" height="4" rx="2" fill="#555"/><rect x="70" y="68" width="50" height="5" rx="2" fill="#4ADE80"/><rect x="180" y="64" width="60" height="14" rx="4" fill="#C9A84C"/><text x="196" y="73" fill="#0A0A0A" font-size="7" font-weight="bold" font-family="sans-serif">Concluído</text><rect x="64" y="118" width="186" height="28" rx="6" fill="#0d2818" stroke="#4ADE8044"/><text x="100" y="129" fill="#4ADE80" font-size="8" font-family="sans-serif">✓ Agendamento confirmado!</text><text x="90" y="140" fill="#4ADE80" font-size="7" font-family="sans-serif">WhatsApp enviado ao cliente</text></svg>`,

      "appt-inprogress": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="35" width="186" height="55" rx="8" fill="#1a1a1a" stroke="#2196F3" stroke-width="1.5"/><rect x="70" y="43" width="60" height="5" rx="2" fill="#EEE"/><rect x="70" y="53" width="90" height="4" rx="2" fill="#555"/><rect x="70" y="63" width="55" height="5" rx="2" fill="#2196F3"/><rect x="160" y="59" width="80" height="14" rx="4" fill="#2196F344"/><text x="170" y="68" fill="#2196F3" font-size="7" font-weight="bold" font-family="sans-serif">Em andamento</text><circle cx="225" cy="112" r="22" fill="#C9A84C"/><text x="215" y="116" fill="#0A0A0A" font-size="8" font-weight="bold" font-family="sans-serif">Concluir</text></svg>`,

      "btn-concluir": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111" opacity=".6"/><rect x="60" y="20" width="160" height="120" rx="12" fill="#1a1a1a" stroke="#C9A84C" stroke-width="1.5"/><text x="108" y="40" fill="#C9A84C" font-size="9" font-weight="bold" font-family="sans-serif">💳 Pagamento</text><text x="90" y="55" fill="#9BA1A6" font-size="7" font-family="sans-serif">João Silva — Corte + Barba</text><text x="96" y="82" fill="#C9A84C" font-size="20" font-weight="bold" font-family="sans-serif">R$ 55,00</text><text x="70" y="100" fill="#666" font-size="7" font-family="sans-serif">FORMA DE PAGAMENTO</text><rect x="70" y="106" width="60" height="18" rx="6" fill="#1E1E1E" stroke="#333"/><text x="83" y="117" fill="#EEE" font-size="7" font-family="sans-serif">Dinheiro</text><rect x="138" y="106" width="42" height="18" rx="6" fill="#C9A84C22" stroke="#C9A84C"/><text x="150" y="117" fill="#C9A84C" font-size="7" font-family="sans-serif">Pix</text><rect x="188" y="106" width="42" height="18" rx="6" fill="#1E1E1E" stroke="#333"/><text x="196" y="117" fill="#EEE" font-size="7" font-family="sans-serif">Cartão</text></svg>`,

      "payment-modal": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="60" y="15" width="160" height="130" rx="12" fill="#1a1a1a" stroke="#2a2a2a"/><text x="108" y="34" fill="#C9A84C" font-size="9" font-weight="bold" font-family="sans-serif">💳 Pagamento</text><rect x="75" y="42" width="130" height="25" rx="6" fill="#0d2818"/><text x="107" y="57" fill="#4ADE80" font-size="12" font-weight="bold" font-family="sans-serif">R$ 55,00</text><rect x="75" y="74" width="60" height="20" rx="6" fill="#1E1E1E" stroke="#333"/><text x="86" y="87" fill="#EEE" font-size="7" font-family="sans-serif">💵 Dinheiro</text><rect x="143" y="74" width="60" height="20" rx="6" fill="#C9A84C22" stroke="#C9A84C" stroke-width="1.5"/><text x="158" y="87" fill="#C9A84C" font-size="7" font-family="sans-serif">⚡ Pix</text><rect x="75" y="100" width="60" height="20" rx="6" fill="#1E1E1E" stroke="#333"/><text x="84" y="113" fill="#EEE" font-size="7" font-family="sans-serif">💳 Crédito</text><rect x="143" y="100" width="60" height="20" rx="6" fill="#1E1E1E" stroke="#333"/><text x="152" y="113" fill="#EEE" font-size="7" font-family="sans-serif">💳 Débito</text><rect x="90" y="128" width="100" height="12" rx="5" fill="#555"/><text x="123" y="136" fill="#AAA" font-size="7" font-family="sans-serif">Cancelar</text></svg>`,

      "payment-done": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="40" width="186" height="55" rx="8" fill="#1a1a1a" stroke="#333"/><rect x="70" y="48" width="60" height="5" rx="2" fill="#EEE"/><rect x="70" y="58" width="80" height="4" rx="2" fill="#555"/><rect x="160" y="44" width="80" height="18" rx="4" fill="#4ADE8022" stroke="#4ADE8044"/><text x="170" y="55" fill="#4ADE80" font-size="8" font-weight="bold" font-family="sans-serif">✓ Concluído</text><rect x="64" y="105" width="186" height="40" rx="8" fill="#0d2818" stroke="#4ADE8044"/><text x="88" y="121" fill="#4ADE80" font-size="9" font-weight="bold" font-family="sans-serif">✓ Pagamento registrado!</text><text x="80" y="133" fill="#4ADE80" font-size="7" font-family="sans-serif">Venda criada no Financeiro · R$ 55,00</text></svg>`,

      "clientes-list": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A"/><rect x="64" y="12" width="80" height="7" rx="3" fill="#EEE"/><rect x="200" y="10" width="55" height="11" rx="5" fill="#C9A84C"/><text x="212" y="19" fill="#0A0A0A" font-size="7" font-weight="bold" font-family="sans-serif">+ Novo</text><rect x="64" y="34" width="186" height="30" rx="6" fill="#1a1a1a" stroke="#333"/><circle cx="82" cy="49" r="8" fill="#C9A84C22" stroke="#C9A84C44"/><rect x="96" y="43" width="60" height="5" rx="2" fill="#EEE"/><rect x="96" y="53" width="80" height="3" rx="1" fill="#555"/><rect x="64" y="70" width="186" height="30" rx="6" fill="#1a1a1a" stroke="#333"/><circle cx="82" cy="85" r="8" fill="#60A5FA22" stroke="#60A5FA44"/><rect x="96" y="79" width="50" height="5" rx="2" fill="#EEE"/><rect x="96" y="89" width="70" height="3" rx="1" fill="#555"/><rect x="64" y="106" width="186" height="30" rx="6" fill="#1a1a1a" stroke="#333"/><circle cx="82" cy="121" r="8" fill="#4ADE8022" stroke="#4ADE8044"/><rect x="96" y="115" width="70" height="5" rx="2" fill="#EEE"/><rect x="96" y="125" width="50" height="3" rx="1" fill="#555"/></svg>`,

      "novo-cliente": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111" opacity=".5"/><rect x="55" y="15" width="170" height="130" rx="12" fill="#1a1a1a" stroke="#C9A84C" stroke-width="1"/><text x="103" y="33" fill="#EEE" font-size="9" font-weight="bold" font-family="sans-serif">Novo Cliente</text><text x="65" y="50" fill="#666" font-size="7" font-family="sans-serif">NOME *</text><rect x="65" y="54" width="150" height="14" rx="4" fill="#222" stroke="#C9A84C" stroke-width="1"/><text x="73" y="64" fill="#EEE" font-size="7" font-family="sans-serif">João da Silva</text><text x="65" y="80" fill="#666" font-size="7" font-family="sans-serif">TELEFONE *</text><rect x="65" y="84" width="150" height="14" rx="4" fill="#222" stroke="#333"/><text x="73" y="94" fill="#EEE" font-size="7" font-family="sans-serif">(16) 99999-9999</text><text x="65" y="110" fill="#666" font-size="7" font-family="sans-serif">E-MAIL (opcional)</text><rect x="65" y="114" width="150" height="14" rx="4" fill="#222" stroke="#333"/><rect x="85" y="136" width="110" height="10" rx="4" fill="#C9A84C"/><text x="118" y="143" fill="#0A0A0A" font-size="7" font-weight="bold" font-family="sans-serif">Salvar</text></svg>`,

      "form-cliente": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111" opacity=".5"/><rect x="55" y="15" width="170" height="130" rx="12" fill="#1a1a1a" stroke="#2a2a2a"/><text x="103" y="33" fill="#EEE" font-size="9" font-weight="bold" font-family="sans-serif">Novo Cliente</text><rect x="65" y="40" width="150" height="14" rx="4" fill="#222" stroke="#C9A84C" stroke-width="1"/><text x="73" y="50" fill="#EEE" font-size="7" font-family="sans-serif">João da Silva</text><rect x="65" y="60" width="150" height="14" rx="4" fill="#222" stroke="#C9A84C" stroke-width="1"/><text x="73" y="70" fill="#EEE" font-size="7" font-family="sans-serif">(16) 99272-2728</text><rect x="65" y="80" width="150" height="14" rx="4" fill="#222" stroke="#333"/><text x="73" y="90" fill="#555" font-size="7" font-family="sans-serif">joao@email.com (opcional)</text><rect x="65" y="100" width="70" height="14" rx="4" fill="#222" stroke="#333"/><text x="73" y="110" fill="#555" font-size="7" font-family="sans-serif">15/03/1990</text><rect x="65" y="120" width="150" height="14" rx="4" fill="#222" stroke="#333"/><text x="73" y="130" fill="#555" font-size="7" font-family="sans-serif">Prefere horários da tarde...</text></svg>`,

      "cliente-salvo": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="30" width="186" height="70" rx="8" fill="#1a1a1a" stroke="#333"/><circle cx="90" cy="65" r="16" fill="#C9A84C22" stroke="#C9A84C44"/><text x="83" y="69" fill="#C9A84C" font-size="11" font-family="sans-serif">JS</text><rect x="114" y="52" width="70" height="6" rx="2" fill="#EEE"/><rect x="114" y="64" width="100" height="4" rx="2" fill="#555"/><rect x="114" y="74" width="80" height="4" rx="2" fill="#555"/><rect x="64" y="110" width="186" height="28" rx="6" fill="#0d2818" stroke="#4ADE8044"/><text x="94" y="121" fill="#4ADE80" font-size="8" font-weight="bold" font-family="sans-serif">✓ Cliente cadastrado com sucesso!</text><text x="84" y="132" fill="#4ADE80" font-size="7" font-family="sans-serif">Já pode agendar pela página pública</text></svg>`,

      "servicos-list": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="12" width="80" height="7" rx="3" fill="#EEE"/><rect x="200" y="10" width="55" height="11" rx="5" fill="#C9A84C"/><rect x="64" y="34" width="88" height="50" rx="8" fill="#1a1a1a" stroke="#333"/><rect x="70" y="40" width="76" height="20" rx="4" fill="#A78BFA22"/><text x="98" y="54" fill="#A78BFA" font-size="14" font-family="sans-serif">✂</text><rect x="70" y="64" width="60" height="5" rx="2" fill="#EEE"/><rect x="70" y="73" width="40" height="4" rx="2" fill="#C9A84C"/><rect x="160" y="34" width="88" height="50" rx="8" fill="#1a1a1a" stroke="#333"/><rect x="166" y="40" width="76" height="20" rx="4" fill="#60A5FA22"/><text x="194" y="54" fill="#60A5FA" font-size="14" font-family="sans-serif">🧔</text><rect x="166" y="64" width="55" height="5" rx="2" fill="#EEE"/><rect x="166" y="73" width="40" height="4" rx="2" fill="#C9A84C"/><rect x="64" y="90" width="88" height="50" rx="8" fill="#1a1a1a" stroke="#333"/><rect x="166" y="90" width="88" height="50" rx="8" fill="#1a1a1a" stroke="#C9A84C" stroke-dasharray="3"/><text x="192" y="119" fill="#C9A84C" font-size="20" font-family="sans-serif">+</text></svg>`,

      "novo-servico": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111" opacity=".5"/><rect x="55" y="10" width="170" height="140" rx="12" fill="#1a1a1a" stroke="#A78BFA" stroke-width="1"/><text x="100" y="28" fill="#EEE" font-size="9" font-weight="bold" font-family="sans-serif">Novo Serviço</text><text x="65" y="42" fill="#666" font-size="7" font-family="sans-serif">NOME DO SERVIÇO</text><rect x="65" y="46" width="150" height="14" rx="4" fill="#222" stroke="#A78BFA" stroke-width="1"/><text x="73" y="56" fill="#EEE" font-size="7" font-family="sans-serif">Corte + Barba</text><text x="65" y="72" fill="#666" font-size="7" font-family="sans-serif">DURAÇÃO &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; PREÇO</text><rect x="65" y="76" width="68" height="14" rx="4" fill="#222" stroke="#333"/><text x="73" y="86" fill="#EEE" font-size="7" font-family="sans-serif">60 min</text><rect x="141" y="76" width="74" height="14" rx="4" fill="#222" stroke="#333"/><text x="149" y="86" fill="#EEE" font-size="7" font-family="sans-serif">R$ 55,00</text><text x="65" y="104" fill="#666" font-size="7" font-family="sans-serif">CATEGORIA</text><rect x="65" y="108" width="150" height="14" rx="4" fill="#222" stroke="#333"/><text x="73" y="118" fill="#EEE" font-size="7" font-family="sans-serif">Corte</text><rect x="80" y="132" width="120" height="12" rx="5" fill="#A78BFA"/><text x="115" y="140" fill="#FFF" font-size="7" font-weight="bold" font-family="sans-serif">Salvar Serviço</text></svg>`,

      "form-servico": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111" opacity=".5"/><rect x="55" y="10" width="170" height="140" rx="12" fill="#1a1a1a" stroke="#2a2a2a"/><rect x="65" y="18" width="150" height="14" rx="4" fill="#222" stroke="#A78BFA" stroke-width="1"/><text x="73" y="28" fill="#EEE" font-size="7" font-family="sans-serif">Corte + Barba</text><rect x="65" y="38" width="68" height="14" rx="4" fill="#222" stroke="#A78BFA" stroke-width="1"/><text x="73" y="48" fill="#EEE" font-size="7" font-family="sans-serif">60 min</text><rect x="141" y="38" width="74" height="14" rx="4" fill="#222" stroke="#A78BFA" stroke-width="1"/><text x="149" y="48" fill="#C9A84C" font-size="8" font-weight="bold" font-family="sans-serif">R$ 55,00</text><rect x="65" y="58" width="150" height="40" rx="6" fill="#1a0a3d" stroke="#A78BFA44"/><text x="115" y="73" fill="#A78BFA" font-size="18" font-family="sans-serif">✂</text><text x="100" y="90" fill="#A78BFA" font-size="7" font-family="sans-serif">+ Adicionar foto</text><rect x="65" y="106" width="150" height="14" rx="4" fill="#222" stroke="#333"/><text x="73" y="116" fill="#EEE" font-size="7" font-family="sans-serif">Corte</text><rect x="65" y="128" width="150" height="16" rx="6" fill="#A78BFA"/><text x="110" y="138" fill="#FFF" font-size="8" font-weight="bold" font-family="sans-serif">Salvar Serviço</text></svg>`,

      "servico-pub": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#0f0f1a"/><rect x="10" y="10" width="260" height="18" rx="4" fill="#1a1a2e"/><text x="95" y="22" fill="#A78BFA" font-size="8" font-weight="bold" font-family="sans-serif">✂ Barbearia Mogiana</text><rect x="10" y="35" width="80" height="90" rx="8" fill="#1a1a1a" stroke="#A78BFA" stroke-width="1"/><rect x="14" y="39" width="72" height="40" rx="4" fill="#A78BFA22"/><text x="42" y="63" fill="#A78BFA" font-size="18" font-family="sans-serif">✂</text><rect x="18" y="83" width="60" height="5" rx="2" fill="#EEE"/><rect x="18" y="92" width="40" height="4" rx="2" fill="#C9A84C"/><rect x="18" y="110" width="64" height="12" rx="5" fill="#A78BFA"/><text x="31" y="118" fill="#FFF" font-size="7" font-family="sans-serif">Agendar</text><rect x="100" y="35" width="80" height="90" rx="8" fill="#1a1a1a" stroke="#333"/><rect x="104" y="39" width="72" height="40" rx="4" fill="#60A5FA22"/><rect x="108" y="83" width="55" height="5" rx="2" fill="#EEE"/><rect x="108" y="92" width="40" height="4" rx="2" fill="#C9A84C"/><rect x="108" y="110" width="64" height="12" rx="5" fill="#444"/><text x="121" y="118" fill="#AAA" font-size="7" font-family="sans-serif">Agendar</text><rect x="190" y="35" width="80" height="90" rx="8" fill="#1a1a1a" stroke="#333"/></svg>`,

      "financeiro-dash": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="12" width="70" height="7" rx="3" fill="#EEE"/><rect x="64" y="32" width="56" height="44" rx="8" fill="#1a1a1a" stroke="#4ADE8044"/><text x="72" y="48" fill="#666" font-size="6" font-family="sans-serif">RECEITAS</text><text x="69" y="62" fill="#4ADE80" font-size="10" font-weight="bold" font-family="sans-serif">R$2.480</text><rect x="128" y="32" width="56" height="44" rx="8" fill="#1a1a1a" stroke="#F8717144"/><text x="136" y="48" fill="#666" font-size="6" font-family="sans-serif">DESPESAS</text><text x="133" y="62" fill="#F87171" font-size="10" font-weight="bold" font-family="sans-serif">R$380</text><rect x="192" y="32" width="58" height="44" rx="8" fill="#1a1a1a" stroke="#C9A84C44"/><text x="200" y="48" fill="#666" font-size="6" font-family="sans-serif">SALDO</text><text x="197" y="62" fill="#C9A84C" font-size="10" font-weight="bold" font-family="sans-serif">R$2.100</text><rect x="64" y="85" width="186" height="60" rx="8" fill="#1a1a1a" stroke="#333"/><rect x="72" y="95" width="20" height="30" rx="2" fill="#4ADE8066"/><rect x="98" y="85" width="20" height="40" rx="2" fill="#4ADE8066"/><rect x="124" y="90" width="20" height="35" rx="2" fill="#4ADE8066"/><rect x="150" y="88" width="20" height="37" rx="2" fill="#4ADE8066"/><rect x="176" y="80" width="20" height="45" rx="2" fill="#C9A84C"/><rect x="202" y="92" width="20" height="33" rx="2" fill="#4ADE8066"/><rect x="228" y="87" width="14" height="38" rx="2" fill="#4ADE8066"/></svg>`,

      "financeiro-cards": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="20" width="56" height="50" rx="8" fill="#0d2818" stroke="#4ADE8044"/><text x="72" y="36" fill="#4ADE80" font-size="6" font-family="sans-serif">RECEITAS</text><text x="69" y="52" fill="#4ADE80" font-size="11" font-weight="bold" font-family="sans-serif">R$2.480</text><text x="69" y="62" fill="#4ADE80" font-size="6" font-family="sans-serif">↑ 18% vs mês anterior</text><rect x="128" y="20" width="56" height="50" rx="8" fill="#1a0808" stroke="#F8717144"/><text x="136" y="36" fill="#F87171" font-size="6" font-family="sans-serif">DESPESAS</text><text x="133" y="52" fill="#F87171" font-size="11" font-weight="bold" font-family="sans-serif">R$380</text><text x="133" y="62" fill="#F87171" font-size="6" font-family="sans-serif">5 lançamentos</text><rect x="192" y="20" width="58" height="50" rx="8" fill="#1a1505" stroke="#C9A84C44"/><text x="200" y="36" fill="#C9A84C" font-size="6" font-family="sans-serif">SALDO LÍQUIDO</text><text x="197" y="52" fill="#C9A84C" font-size="11" font-weight="bold" font-family="sans-serif">R$2.100</text><text x="197" y="62" fill="#C9A84C" font-size="6" font-family="sans-serif">Lucro do período</text><rect x="64" y="82" width="186" height="60" rx="8" fill="#1a1a1a" stroke="#333"/><rect x="72" y="92" width="170" height="6" rx="2" fill="#222"/><rect x="72" y="104" width="170" height="6" rx="2" fill="#222"/><rect x="72" y="116" width="170" height="6" rx="2" fill="#222"/><rect x="72" y="128" width="170" height="6" rx="2" fill="#222"/></svg>`,

      "financeiro-filtros": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="15" width="186" height="22" rx="8" fill="#1a1a1a" stroke="#333"/><rect x="70" y="19" width="35" height="14" rx="5" fill="#333"/><text x="76" y="28" fill="#AAA" font-size="7" font-family="sans-serif">Hoje</text><rect x="111" y="19" width="46" height="14" rx="5" fill="#333"/><text x="115" y="28" fill="#AAA" font-size="7" font-family="sans-serif">Semana</text><rect x="163" y="19" width="35" height="14" rx="5" fill="#C9A84C"/><text x="169" y="28" fill="#0A0A0A" font-size="7" font-weight="bold" font-family="sans-serif">Mês</text><rect x="204" y="19" width="40" height="14" rx="5" fill="#333"/><text x="208" y="28" fill="#AAA" font-size="7" font-family="sans-serif">Período</text><rect x="64" y="46" width="56" height="44" rx="8" fill="#0d2818" stroke="#4ADE8044"/><text x="72" y="60" fill="#4ADE80" font-size="6" font-family="sans-serif">RECEITAS</text><text x="69" y="76" fill="#4ADE80" font-size="11" font-weight="bold" font-family="sans-serif">R$2.480</text><rect x="128" y="46" width="56" height="44" rx="8" fill="#1a0808" stroke="#F8717144"/><text x="136" y="60" fill="#F87171" font-size="6" font-family="sans-serif">DESPESAS</text><text x="133" y="76" fill="#F87171" font-size="11" font-weight="bold" font-family="sans-serif">R$380</text><rect x="192" y="46" width="58" height="44" rx="8" fill="#1a1505" stroke="#C9A84C44"/><text x="200" y="60" fill="#C9A84C" font-size="6" font-family="sans-serif">SALDO</text><text x="197" y="76" fill="#C9A84C" font-size="11" font-weight="bold" font-family="sans-serif">R$2.100</text></svg>`,

      "financeiro-export": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="12" width="80" height="7" rx="3" fill="#EEE"/><rect x="164" y="10" width="40" height="11" rx="5" fill="#1a1a1a" stroke="#333"/><text x="172" y="18" fill="#AAA" font-size="7" font-family="sans-serif">📄 PDF</text><rect x="210" y="10" width="40" height="11" rx="5" fill="#C9A84C22" stroke="#C9A84C"/><text x="218" y="18" fill="#C9A84C" font-size="7" font-family="sans-serif">📊 CSV</text><rect x="64" y="32" width="186" height="110" rx="8" fill="#1a1a1a" stroke="#333"/><rect x="72" y="42" width="80" height="5" rx="2" fill="#EEE"/><rect x="72" y="52" width="60" height="4" rx="2" fill="#555"/><rect x="72" y="65" width="170" height="1" fill="#222"/><rect x="72" y="72" width="100" height="4" rx="2" fill="#555"/><rect x="202" y="70" width="40" height="6" rx="2" fill="#4ADE8022"/><text x="212" y="76" fill="#4ADE80" font-size="5" font-family="sans-serif">R$55,00</text><rect x="72" y="84" width="100" height="4" rx="2" fill="#555"/><rect x="202" y="82" width="40" height="6" rx="2" fill="#4ADE8022"/><text x="212" y="88" fill="#4ADE80" font-size="5" font-family="sans-serif">R$35,00</text><rect x="72" y="96" width="80" height="4" rx="2" fill="#555"/><rect x="202" y="94" width="40" height="6" rx="2" fill="#F8717122"/><text x="210" y="100" fill="#F87171" font-size="5" font-family="sans-serif">-R$120,00</text></svg>`,

      "pagina-preview": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#0f1117"/><rect x="10" y="10" width="260" height="22" rx="6" fill="#1a1a2e"/><circle cx="30" cy="21" r="8" fill="#C9A84C22" stroke="#C9A84C44"/><text x="24" y="25" fill="#C9A84C" font-size="8" font-family="sans-serif">✂</text><text x="44" y="24" fill="#EEE" font-size="9" font-weight="bold" font-family="sans-serif">Mogiana Barber</text><rect x="200" y="14" width="60" height="14" rx="5" fill="#C9A84C"/><text x="210" y="23" fill="#0A0A0A" font-size="7" font-weight="bold" font-family="sans-serif">Agendar →</text><rect x="10" y="38" width="260" height="50" rx="0" fill="#1a1505"/><text x="20" y="58" fill="#EEE" font-size="11" font-weight="bold" font-family="sans-serif">Bem-vindo à</text><text x="20" y="72" fill="#C9A84C" font-size="11" font-weight="bold" font-family="sans-serif">Mogiana Barber</text><rect x="10" y="96" width="84" height="55" rx="8" fill="#1a1a1a" stroke="#333"/><rect x="100" y="96" width="84" height="55" rx="8" fill="#1a1a1a" stroke="#333"/><rect x="190" y="96" width="80" height="55" rx="8" fill="#1a1a1a" stroke="#333"/><rect x="16" y="102" width="72" height="25" rx="4" fill="#A78BFA22"/><text x="42" y="118" fill="#A78BFA" font-size="14" font-family="sans-serif">✂</text><rect x="16" y="131" width="60" height="5" rx="2" fill="#EEE"/><rect x="16" y="140" width="40" height="4" rx="2" fill="#C9A84C"/></svg>`,

      "pagina-config": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="12" width="80" height="7" rx="3" fill="#EEE"/><rect x="64" y="30" width="186" height="115" rx="8" fill="#1a1a1a" stroke="#333"/><text x="75" y="48" fill="#666" font-size="7" font-family="sans-serif">COR PRINCIPAL</text><rect x="75" y="52" width="120" height="14" rx="4" fill="#C9A84C22" stroke="#C9A84C"/><text x="83" y="62" fill="#C9A84C" font-size="7" font-family="sans-serif">#C9A84C — Dourado</text><rect x="201" y="52" width="40" height="14" rx="4" fill="#C9A84C"/><text x="75" y="80" fill="#666" font-size="7" font-family="sans-serif">LOGO DA BARBEARIA</text><rect x="75" y="84" width="60" height="30" rx="4" fill="#222" stroke="#333" stroke-dasharray="3"/><text x="85" y="102" fill="#555" font-size="7" font-family="sans-serif">+ Upload</text><text x="145" y="80" fill="#666" font-size="7" font-family="sans-serif">FOTO DE CAPA</text><rect x="145" y="84" width="96" height="30" rx="4" fill="#222" stroke="#333" stroke-dasharray="3"/><text x="175" y="102" fill="#555" font-size="7" font-family="sans-serif">+ Upload</text><rect x="90" y="126" width="120" height="12" rx="5" fill="#C9A84C"/><text x="122" y="134" fill="#0A0A0A" font-size="7" font-weight="bold" font-family="sans-serif">Salvar</text></svg>`,

      "pagina-horarios": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="12" width="80" height="7" rx="3" fill="#EEE"/><rect x="64" y="30" width="186" height="118" rx="8" fill="#1a1a1a" stroke="#333"/><rect x="74" y="40" width="50" height="12" rx="4" fill="#4ADE8022" stroke="#4ADE8044"/><text x="80" y="48" fill="#4ADE80" font-size="7" font-family="sans-serif">Segunda</text><rect x="74" y="58" width="50" height="12" rx="4" fill="#4ADE8022" stroke="#4ADE8044"/><text x="82" y="66" fill="#4ADE80" font-size="7" font-family="sans-serif">Terça</text><rect x="74" y="76" width="50" height="12" rx="4" fill="#4ADE8022" stroke="#4ADE8044"/><text x="79" y="84" fill="#4ADE80" font-size="7" font-family="sans-serif">Quarta</text><rect x="74" y="94" width="50" height="12" rx="4" fill="#4ADE8022" stroke="#4ADE8044"/><text x="80" y="102" fill="#4ADE80" font-size="7" font-family="sans-serif">Quinta</text><rect x="74" y="112" width="50" height="12" rx="4" fill="#4ADE8022" stroke="#4ADE8044"/><text x="82" y="120" fill="#4ADE80" font-size="7" font-family="sans-serif">Sexta</text><text x="134" y="50" fill="#666" font-size="7" font-family="sans-serif">08:00 → 18:00</text><text x="134" y="68" fill="#666" font-size="7" font-family="sans-serif">08:00 → 18:00</text><text x="134" y="86" fill="#666" font-size="7" font-family="sans-serif">08:00 → 18:00</text><text x="134" y="104" fill="#666" font-size="7" font-family="sans-serif">08:00 → 18:00</text><text x="134" y="122" fill="#666" font-size="7" font-family="sans-serif">08:00 → 20:00</text><rect x="74" y="130" width="50" height="12" rx="4" fill="#F8717122" stroke="#F8717144"/><text x="80" y="138" fill="#F87171" font-size="7" font-family="sans-serif">Domingo ✕</text></svg>`,

      "pagina-link": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="25" width="186" height="50" rx="8" fill="#1a1505" stroke="#C9A84C44"/><text x="75" y="43" fill="#666" font-size="7" font-family="sans-serif">SEU LINK DE AGENDAMENTO</text><rect x="75" y="48" width="140" height="18" rx="4" fill="#111" stroke="#C9A84C"/><text x="82" y="60" fill="#C9A84C" font-size="7" font-family="sans-serif">usebarberpro.com/pub/mogiana</text><rect x="221" y="48" width="24" height="18" rx="4" fill="#C9A84C"/><text x="226" y="60" fill="#0A0A0A" font-size="10" font-family="sans-serif">⎘</text><rect x="64" y="86" width="88" height="35" rx="8" fill="#1a1a1a" stroke="#25D366"/><text x="78" y="102" fill="#25D366" font-size="8" font-family="sans-serif">💬 WhatsApp</text><text x="76" y="113" fill="#555" font-size="6" font-family="sans-serif">Compartilhar link</text><rect x="162" y="86" width="88" height="35" rx="8" fill="#1a1a1a" stroke="#E1306C"/><text x="175" y="102" fill="#E1306C" font-size="8" font-family="sans-serif">📸 Instagram</text><text x="174" y="113" fill="#555" font-size="6" font-family="sans-serif">Colocar na bio</text><rect x="64" y="130" width="186" height="18" rx="6" fill="#0d2818" stroke="#4ADE8044"/><text x="104" y="141" fill="#4ADE80" font-size="7" font-weight="bold" font-family="sans-serif">✓ Página pública ativa!</text></svg>`,

      // Estoque
      "estoque-list": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="12" width="80" height="7" rx="3" fill="#EEE"/><rect x="64" y="32" width="186" height="34" rx="6" fill="#1a1a1a" stroke="#FBBF2444"/><rect x="70" y="39" width="30" height="5" rx="2" fill="#EEE"/><rect x="70" y="48" width="50" height="4" rx="2" fill="#FBBF24"/><rect x="180" y="37" width="28" height="12" rx="4" fill="#4ADE8022" stroke="#4ADE8044"/><text x="186" y="45" fill="#4ADE80" font-size="7" font-family="sans-serif">25un</text><rect x="214" y="37" width="28" height="12" rx="4" fill="#C9A84C"/><text x="220" y="45" fill="#0A0A0A" font-size="7" font-weight="bold" font-family="sans-serif">Entrada</text><rect x="64" y="72" width="186" height="34" rx="6" fill="#1a1a1a" stroke="#F8717144"/><rect x="70" y="79" width="40" height="5" rx="2" fill="#EEE"/><rect x="70" y="88" width="50" height="4" rx="2" fill="#F87171"/><rect x="180" y="77" width="28" height="12" rx="4" fill="#F8717122" stroke="#F8717144"/><text x="187" y="85" fill="#F87171" font-size="7" font-family="sans-serif">2un</text><rect x="214" y="77" width="28" height="12" rx="4" fill="#C9A84C"/><text x="220" y="85" fill="#0A0A0A" font-size="7" font-weight="bold" font-family="sans-serif">Entrada</text><rect x="64" y="112" width="186" height="34" rx="6" fill="#1a1a1a" stroke="#333"/><rect x="70" y="119" width="55" height="5" rx="2" fill="#EEE"/><rect x="70" y="128" width="40" height="4" rx="2" fill="#555"/><rect x="180" y="117" width="28" height="12" rx="4" fill="#4ADE8022" stroke="#4ADE8044"/><text x="186" y="125" fill="#4ADE80" font-size="7" font-family="sans-serif">10un</text></svg>`,

      "estoque-entrada": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111" opacity=".6"/><rect x="55" y="15" width="170" height="130" rx="12" fill="#1a1a1a" stroke="#FBBF24" stroke-width="1"/><text x="100" y="33" fill="#FBBF24" font-size="9" font-weight="bold" font-family="sans-serif">📦 Entrada de Estoque</text><text x="65" y="50" fill="#666" font-size="7" font-family="sans-serif">PRODUTO</text><rect x="65" y="54" width="150" height="14" rx="4" fill="#222" stroke="#333"/><text x="73" y="64" fill="#EEE" font-size="7" font-family="sans-serif">Pomada Cabelo Black</text><text x="65" y="80" fill="#666" font-size="7" font-family="sans-serif">QUANTIDADE RECEBIDA</text><rect x="65" y="84" width="70" height="14" rx="4" fill="#222" stroke="#FBBF24" stroke-width="1"/><text x="73" y="94" fill="#FBBF24" font-size="9" font-weight="bold" font-family="sans-serif">24</text><text x="65" y="110" fill="#666" font-size="7" font-family="sans-serif">FORNECEDOR</text><rect x="65" y="114" width="150" height="14" rx="4" fill="#222" stroke="#333"/><text x="73" y="124" fill="#EEE" font-size="7" font-family="sans-serif">Distribuidora ABC</text><rect x="80" y="136" width="120" height="10" rx="4" fill="#FBBF24"/><text x="116" y="143" fill="#0A0A0A" font-size="7" font-weight="bold" font-family="sans-serif">Confirmar</text></svg>`,

      "estoque-alerta": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="20" width="186" height="40" rx="6" fill="#1a0808" stroke="#F8717166"/><text x="72" y="35" fill="#F87171" font-size="8" font-weight="bold" font-family="sans-serif">⚠ Estoque crítico: Pomada Black</text><text x="72" y="47" fill="#F87171" font-size="7" font-family="sans-serif">Apenas 2 unidades — mínimo definido: 5</text><rect x="64" y="68" width="186" height="34" rx="6" fill="#1a0f00" stroke="#FBBF2444"/><rect x="70" y="75" width="40" height="5" rx="2" fill="#EEE"/><rect x="70" y="84" width="60" height="4" rx="2" fill="#FBBF24"/><rect x="180" y="73" width="28" height="12" rx="4" fill="#F8717122" stroke="#F8717144"/><text x="187" y="81" fill="#F87171" font-size="7" font-weight="bold" font-family="sans-serif">2un</text><rect x="64" y="108" width="186" height="34" rx="6" fill="#1a1a1a" stroke="#4ADE8044"/><rect x="70" y="115" width="55" height="5" rx="2" fill="#EEE"/><rect x="70" y="124" width="40" height="4" rx="2" fill="#555"/><rect x="180" y="113" width="28" height="12" rx="4" fill="#4ADE8022" stroke="#4ADE8044"/><text x="186" y="121" fill="#4ADE80" font-size="7" font-family="sans-serif">25un</text></svg>`,

      "estoque-historico": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="12" width="130" height="7" rx="3" fill="#EEE"/><rect x="64" y="32" width="186" height="110" rx="8" fill="#1a1a1a" stroke="#333"/><rect x="72" y="44" width="170" height="1" fill="#222"/><text x="72" y="42" fill="#666" font-size="6" font-family="sans-serif">DATA</text><text x="110" y="42" fill="#666" font-size="6" font-family="sans-serif">TIPO</text><text x="150" y="42" fill="#666" font-size="6" font-family="sans-serif">QTD</text><text x="190" y="42" fill="#666" font-size="6" font-family="sans-serif">RESPONSÁVEL</text><text x="72" y="56" fill="#EEE" font-size="7" font-family="sans-serif">26/05/2026</text><rect x="108" y="50" width="30" height="9" rx="3" fill="#4ADE8022"/><text x="112" y="57" fill="#4ADE80" font-size="6" font-family="sans-serif">Entrada</text><text x="150" y="56" fill="#4ADE80" font-size="7" font-family="sans-serif">+24</text><text x="190" y="56" fill="#EEE" font-size="7" font-family="sans-serif">Carlos</text><text x="72" y="74" fill="#EEE" font-size="7" font-family="sans-serif">20/05/2026</text><rect x="108" y="68" width="25" height="9" rx="3" fill="#F8717122"/><text x="111" y="75" fill="#F87171" font-size="6" font-family="sans-serif">Saída</text><text x="150" y="74" fill="#F87171" font-size="7" font-family="sans-serif">-3</text><text x="190" y="74" fill="#EEE" font-size="7" font-family="sans-serif">Venda</text><text x="72" y="92" fill="#EEE" font-size="7" font-family="sans-serif">15/05/2026</text><rect x="108" y="86" width="30" height="9" rx="3" fill="#4ADE8022"/><text x="112" y="93" fill="#4ADE80" font-size="6" font-family="sans-serif">Entrada</text><text x="150" y="92" fill="#4ADE80" font-size="7" font-family="sans-serif">+12</text><text x="190" y="92" fill="#EEE" font-size="7" font-family="sans-serif">Carlos</text></svg>`,

      // Assinaturas
      "planos-list": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="12" width="90" height="7" rx="3" fill="#EEE"/><rect x="200" y="10" width="55" height="11" rx="5" fill="#C9A84C"/><rect x="64" y="30" width="88" height="70" rx="8" fill="#1a1a1a" stroke="#A78BFA" stroke-width="1.5"/><text x="80" y="46" fill="#A78BFA" font-size="8" font-weight="bold" font-family="sans-serif">Plano Básico</text><text x="74" y="58" fill="#C9A84C" font-size="11" font-weight="bold" font-family="sans-serif">R$79/mês</text><rect x="70" y="65" width="55" height="4" rx="2" fill="#333"/><rect x="70" y="73" width="45" height="4" rx="2" fill="#333"/><rect x="70" y="86" width="76" height="10" rx="4" fill="#A78BFA22"/><text x="85" y="93" fill="#A78BFA" font-size="7" font-family="sans-serif">8 assinantes</text><rect x="160" y="30" width="90" height="70" rx="8" fill="#1a1505" stroke="#C9A84C" stroke-width="1.5"/><text x="174" y="46" fill="#C9A84C" font-size="8" font-weight="bold" font-family="sans-serif">Plano Premium</text><text x="168" y="58" fill="#C9A84C" font-size="11" font-weight="bold" font-family="sans-serif">R$149/mês</text><rect x="166" y="65" width="70" height="4" rx="2" fill="#333"/><rect x="166" y="73" width="55" height="4" rx="2" fill="#333"/><rect x="166" y="86" width="76" height="10" rx="4" fill="#C9A84C22"/><text x="181" y="93" fill="#C9A84C" font-size="7" font-family="sans-serif">3 assinantes</text><rect x="64" y="115" width="186" height="28" rx="6" fill="#1a1a1a" stroke="#333"/><rect x="70" y="122" width="80" height="5" rx="2" fill="#EEE"/><rect x="70" y="131" width="60" height="4" rx="2" fill="#555"/><text x="218" y="134" fill="#C9A84C" font-size="14" font-family="sans-serif">›</text></svg>`,

      "plano-novo": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111" opacity=".6"/><rect x="50" y="10" width="180" height="140" rx="12" fill="#1a1a1a" stroke="#A78BFA" stroke-width="1"/><text x="100" y="28" fill="#A78BFA" font-size="9" font-weight="bold" font-family="sans-serif">Novo Plano</text><text x="60" y="44" fill="#666" font-size="7" font-family="sans-serif">NOME DO PLANO</text><rect x="60" y="48" width="160" height="14" rx="4" fill="#222" stroke="#A78BFA" stroke-width="1"/><text x="68" y="58" fill="#EEE" font-size="7" font-family="sans-serif">Plano Premium</text><text x="60" y="74" fill="#666" font-size="7" font-family="sans-serif">PREÇO MENSAL</text><rect x="60" y="78" width="80" height="14" rx="4" fill="#222" stroke="#333"/><text x="68" y="88" fill="#C9A84C" font-size="9" font-weight="bold" font-family="sans-serif">R$ 149</text><text x="60" y="104" fill="#666" font-size="7" font-family="sans-serif">DESCRIÇÃO</text><rect x="60" y="108" width="160" height="24" rx="4" fill="#222" stroke="#333"/><text x="68" y="120" fill="#555" font-size="7" font-family="sans-serif">Cortes ilimitados + produtos</text><rect x="75" y="140" width="130" height="10" rx="4" fill="#A78BFA"/><text x="118" y="147" fill="#FFF" font-size="7" font-weight="bold" font-family="sans-serif">Próximo</text></svg>`,

      "plano-servicos": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111" opacity=".6"/><rect x="50" y="10" width="180" height="140" rx="12" fill="#1a1a1a" stroke="#2a2a2a"/><text x="85" y="28" fill="#EEE" font-size="9" font-weight="bold" font-family="sans-serif">Serviços incluídos</text><rect x="60" y="36" width="160" height="16" rx="4" fill="#A78BFA22" stroke="#A78BFA44"/><text x="68" y="47" fill="#A78BFA" font-size="7" font-family="sans-serif">✓ Corte de Cabelo</text><text x="196" y="47" fill="#A78BFA" font-size="8" font-family="sans-serif">∞</text><rect x="60" y="58" width="160" height="16" rx="4" fill="#A78BFA22" stroke="#A78BFA44"/><text x="68" y="69" fill="#A78BFA" font-size="7" font-family="sans-serif">✓ Barba</text><text x="196" y="69" fill="#A78BFA" font-size="8" font-family="sans-serif">2×</text><rect x="60" y="80" width="160" height="16" rx="4" fill="#1E1E1E" stroke="#333"/><text x="68" y="91" fill="#666" font-size="7" font-family="sans-serif">○ Sobrancelha</text><rect x="60" y="104" width="160" height="16" rx="4" fill="#C9A84C22" stroke="#C9A84C44"/><text x="68" y="115" fill="#C9A84C" font-size="7" font-family="sans-serif">+ Pomada Cabelo (produto)</text><rect x="75" y="128" width="130" height="14" rx="6" fill="#A78BFA"/><text x="112" y="137" fill="#FFF" font-size="7" font-weight="bold" font-family="sans-serif">Salvar Plano</text></svg>`,

      "plano-pub": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#0f1117"/><rect x="10" y="10" width="260" height="18" rx="4" fill="#1a1a2e"/><text x="95" y="22" fill="#EEE" font-size="8" font-weight="bold" font-family="sans-serif">🌟 Assinaturas</text><rect x="10" y="36" width="120" height="100" rx="8" fill="#1a1a1a" stroke="#A78BFA" stroke-width="1.5"/><text x="30" y="54" fill="#A78BFA" font-size="9" font-weight="bold" font-family="sans-serif">Plano Básico</text><text x="25" y="68" fill="#C9A84C" font-size="12" font-weight="bold" font-family="sans-serif">R$79/mês</text><rect x="18" y="74" width="80" height="4" rx="2" fill="#333"/><rect x="18" y="82" width="65" height="4" rx="2" fill="#333"/><rect x="18" y="90" width="75" height="4" rx="2" fill="#333"/><rect x="25" y="118" width="88" height="14" rx="5" fill="#A78BFA"/><text x="47" y="127" fill="#FFF" font-size="7" font-weight="bold" font-family="sans-serif">Assinar agora</text><rect x="148" y="36" width="122" height="100" rx="8" fill="#1a1505" stroke="#C9A84C" stroke-width="2"/><rect x="168" y="30" width="60" height="12" rx="6" fill="#C9A84C"/><text x="178" y="38" fill="#0A0A0A" font-size="7" font-weight="bold" font-family="sans-serif">Mais popular</text><text x="168" y="54" fill="#C9A84C" font-size="9" font-weight="bold" font-family="sans-serif">Plano Premium</text><text x="158" y="68" fill="#C9A84C" font-size="12" font-weight="bold" font-family="sans-serif">R$149/mês</text><rect x="155" y="118" width="108" height="14" rx="5" fill="#C9A84C"/><text x="183" y="127" fill="#0A0A0A" font-size="7" font-weight="bold" font-family="sans-serif">Assinar agora</text></svg>`,

      // Fidelidade
      "fidelidade-config": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="12" width="100" height="7" rx="3" fill="#EEE"/><rect x="64" y="30" width="186" height="110" rx="8" fill="#1a1a1a" stroke="#F59E0B44"/><text x="75" y="48" fill="#F59E0B" font-size="8" font-weight="bold" font-family="sans-serif">⭐ Programa de Fidelidade</text><rect x="75" y="55" width="160" height="1" fill="#222"/><text x="75" y="70" fill="#666" font-size="7" font-family="sans-serif">PONTOS POR R$ 1,00 GASTO</text><rect x="75" y="74" width="70" height="14" rx="4" fill="#222" stroke="#F59E0B"/><text x="83" y="84" fill="#F59E0B" font-size="9" font-weight="bold" font-family="sans-serif">10 pts</text><text x="75" y="102" fill="#666" font-size="7" font-family="sans-serif">VALIDADE DOS PONTOS</text><rect x="75" y="106" width="70" height="14" rx="4" fill="#222" stroke="#333"/><text x="83" y="116" fill="#EEE" font-size="7" font-family="sans-serif">365 dias</text><rect x="164" y="74" width="70" height="46" rx="6" fill="#F59E0B12" stroke="#F59E0B33"/><text x="174" y="92" fill="#F59E0B" font-size="20" font-family="sans-serif">⭐</text><text x="172" y="110" fill="#F59E0B" font-size="6" font-family="sans-serif">Programa ativo</text><rect x="90" y="128" width="110" height="10" rx="4" fill="#F59E0B"/><text x="123" y="135" fill="#0A0A0A" font-size="7" font-weight="bold" font-family="sans-serif">Salvar</text></svg>`,

      "fidelidade-regras": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="12" width="90" height="7" rx="3" fill="#EEE"/><rect x="64" y="30" width="186" height="50" rx="6" fill="#1a1a1a" stroke="#F59E0B44"/><text x="72" y="44" fill="#666" font-size="6" font-family="sans-serif">RECOMPENSA</text><text x="72" y="55" fill="#EEE" font-size="8" font-weight="bold" font-family="sans-serif">Corte Grátis</text><text x="72" y="65" fill="#F59E0B" font-size="7" font-family="sans-serif">500 pontos</text><rect x="200" y="38" width="44" height="10" rx="4" fill="#F59E0B22" stroke="#F59E0B44"/><text x="208" y="45" fill="#F59E0B" font-size="6" font-family="sans-serif">Ativo</text><rect x="64" y="86" width="186" height="50" rx="6" fill="#1a1a1a" stroke="#F59E0B44"/><text x="72" y="100" fill="#666" font-size="6" font-family="sans-serif">RECOMPENSA</text><text x="72" y="111" fill="#EEE" font-size="8" font-weight="bold" font-family="sans-serif">10% de desconto</text><text x="72" y="121" fill="#F59E0B" font-size="7" font-family="sans-serif">200 pontos</text><rect x="200" y="94" width="44" height="10" rx="4" fill="#F59E0B22" stroke="#F59E0B44"/><text x="208" y="101" fill="#F59E0B" font-size="6" font-family="sans-serif">Ativo</text><rect x="64" y="144" width="186" height="12" rx="4" fill="#C9A84C22" stroke="#C9A84C44"/><text x="110" y="152" fill="#C9A84C" font-size="7" font-family="sans-serif">+ Adicionar recompensa</text></svg>`,

      "fidelidade-pontos": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="20" width="186" height="60" rx="8" fill="#1a1a1a" stroke="#333"/><circle cx="90" cy="50" r="16" fill="#F59E0B22" stroke="#F59E0B44"/><text x="83" y="54" fill="#F59E0B" font-size="11" font-family="sans-serif">JS</text><rect x="114" y="34" width="70" height="5" rx="2" fill="#EEE"/><rect x="114" y="44" width="50" height="4" rx="2" fill="#555"/><rect x="114" y="56" width="100" height="12" rx="4" fill="#F59E0B22"/><text x="122" y="64" fill="#F59E0B" font-size="8" font-weight="bold" font-family="sans-serif">⭐ 340 pontos</text><rect x="64" y="90" width="186" height="28" rx="6" fill="#0d2818" stroke="#4ADE8044"/><text x="80" y="102" fill="#4ADE80" font-size="8" font-weight="bold" font-family="sans-serif">✓ +55 pontos adicionados!</text><text x="72" y="112" fill="#4ADE80" font-size="7" font-family="sans-serif">Venda de R$55,00 · Corte + Barba</text><rect x="64" y="126" width="186" height="24" rx="6" fill="#1a1a1a" stroke="#333"/><rect x="72" y="134" width="80" height="4" rx="2" fill="#555"/><text x="220" y="140" fill="#C9A84C" font-size="14" font-family="sans-serif">›</text></svg>`,

      "fidelidade-resgate": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111" opacity=".6"/><rect x="55" y="10" width="170" height="140" rx="12" fill="#1a1a1a" stroke="#F59E0B" stroke-width="1"/><text x="95" y="28" fill="#F59E0B" font-size="9" font-weight="bold" font-family="sans-serif">⭐ Resgatar Pontos</text><text x="65" y="42" fill="#666" font-size="7" font-family="sans-serif">SALDO ATUAL</text><text x="65" y="55" fill="#F59E0B" font-size="16" font-weight="bold" font-family="sans-serif">340 pts</text><rect x="65" y="64" width="150" height="1" fill="#222"/><rect x="65" y="72" width="150" height="22" rx="6" fill="#1E1E1E" stroke="#333"/><text x="73" y="84" fill="#EEE" font-size="8" font-family="sans-serif">Corte Grátis</text><text x="180" y="84" fill="#F59E0B" font-size="7" font-weight="bold" font-family="sans-serif">500 pts</text><rect x="65" y="100" width="150" height="22" rx="6" fill="#F59E0B22" stroke="#F59E0B44"/><text x="73" y="112" fill="#EEE" font-size="8" font-family="sans-serif">10% desconto</text><text x="180" y="112" fill="#F59E0B" font-size="7" font-weight="bold" font-family="sans-serif">200 pts</text><rect x="80" y="132" width="120" height="14" rx="6" fill="#F59E0B"/><text x="110" y="141" fill="#0A0A0A" font-size="7" font-weight="bold" font-family="sans-serif">Resgatar 200pts</text></svg>`,

      // Avaliações
      "avaliacoes-list": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="12" width="80" height="7" rx="3" fill="#EEE"/><rect x="164" y="10" width="86" height="11" rx="5" fill="#F59E0B22" stroke="#F59E0B44"/><text x="176" y="18" fill="#F59E0B" font-size="8" font-weight="bold" font-family="sans-serif">★ 4.8 média</text><rect x="64" y="32" width="186" height="36" rx="6" fill="#1a1a1a" stroke="#333"/><circle cx="82" cy="50" r="9" fill="#C9A84C22" stroke="#C9A84C44"/><text x="78" y="54" fill="#C9A84C" font-size="8" font-family="sans-serif">JS</text><rect x="98" y="40" width="55" height="5" rx="2" fill="#EEE"/><text x="98" y="54" fill="#F59E0B" font-size="10" font-family="sans-serif">★★★★★</text><rect x="98" y="58" width="100" height="4" rx="2" fill="#555"/><rect x="64" y="74" width="186" height="36" rx="6" fill="#1a1a1a" stroke="#333"/><circle cx="82" cy="92" r="9" fill="#60A5FA22" stroke="#60A5FA44"/><text x="78" y="96" fill="#60A5FA" font-size="8" font-family="sans-serif">MR</text><rect x="98" y="82" width="50" height="5" rx="2" fill="#EEE"/><text x="98" y="96" fill="#F59E0B" font-size="10" font-family="sans-serif">★★★★</text><rect x="98" y="100" width="120" height="4" rx="2" fill="#555"/><rect x="64" y="116" width="186" height="36" rx="6" fill="#1a1a1a" stroke="#333"/><circle cx="82" cy="134" r="9" fill="#4ADE8022" stroke="#4ADE8044"/><text x="78" y="138" fill="#4ADE80" font-size="8" font-family="sans-serif">KS</text><rect x="98" y="124" width="60" height="5" rx="2" fill="#EEE"/><text x="98" y="138" fill="#F59E0B" font-size="10" font-family="sans-serif">★★★★★</text></svg>`,

      "avaliacoes-auto": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="64" y="15" width="186" height="55" rx="8" fill="#1a1a1a" stroke="#333"/><rect x="72" y="22" width="60" height="5" rx="2" fill="#EEE"/><rect x="72" y="32" width="80" height="4" rx="2" fill="#555"/><rect x="160" y="18" width="80" height="18" rx="4" fill="#4ADE8022" stroke="#4ADE8044"/><text x="170" y="29" fill="#4ADE80" font-size="8" font-weight="bold" font-family="sans-serif">✓ Concluído</text><rect x="64" y="82" width="186" height="60" rx="8" fill="#0d2020" stroke="#25D36644"/><text x="76" y="97" fill="#25D366" font-size="8" font-weight="bold" font-family="sans-serif">💬 WhatsApp automático</text><rect x="76" y="104" width="162" height="30" rx="6" fill="#1a2a1a"/><text x="84" y="116" fill="#EEE" font-size="7" font-family="sans-serif">Olá João! Como foi sua experiência</text><text x="84" y="127" fill="#EEE" font-size="7" font-family="sans-serif">na Mogiana? ⭐⭐⭐⭐⭐</text></svg>`,

      "avaliacoes-resposta": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="15" width="186" height="50" rx="8" fill="#1a1a1a" stroke="#333"/><circle cx="82" cy="40" r="9" fill="#C9A84C22" stroke="#C9A84C44"/><text x="78" y="44" fill="#C9A84C" font-size="8" font-family="sans-serif">JS</text><text x="96" y="33" fill="#EEE" font-size="8" font-weight="bold" font-family="sans-serif">João Silva</text><text x="96" y="43" fill="#F59E0B" font-size="10" font-family="sans-serif">★★★★★</text><rect x="96" y="50" width="140" height="4" rx="2" fill="#555"/><rect x="64" y="75" width="186" height="45" rx="8" fill="#C9A84C12" stroke="#C9A84C33"/><text x="74" y="89" fill="#666" font-size="6" font-family="sans-serif">SUA RESPOSTA</text><rect x="72" y="93" width="150" height="20" rx="4" fill="#222" stroke="#C9A84C" stroke-width="1"/><text x="80" y="100" fill="#555" font-size="7" font-family="sans-serif">Obrigado, João! Fico feliz que...</text><rect x="196" y="108" width="46" height="8" rx="3" fill="#C9A84C"/><text x="203" y="114" fill="#0A0A0A" font-size="6" font-weight="bold" font-family="sans-serif">Responder</text></svg>`,

      "avaliacoes-pub": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#0f1117"/><rect x="10" y="10" width="260" height="18" rx="4" fill="#1a1a2e"/><text x="100" y="22" fill="#EEE" font-size="9" font-weight="bold" font-family="sans-serif">✂ Mogiana Barber</text><rect x="10" y="35" width="260" height="35" rx="6" fill="#1a1505" stroke="#F59E0B33"/><text x="20" y="50" fill="#F59E0B" font-size="16" font-family="sans-serif">★★★★★</text><text x="120" y="50" fill="#EEE" font-size="11" font-weight="bold" font-family="sans-serif">4.8</text><text x="145" y="50" fill="#555" font-size="9" font-family="sans-serif">/ 5.0</text><text x="20" y="62" fill="#555" font-size="7" font-family="sans-serif">Baseado em 47 avaliações verificadas</text><rect x="10" y="78" width="124" height="70" rx="8" fill="#1a1a1a" stroke="#333"/><text x="20" y="93" fill="#EEE" font-size="7" font-weight="bold" font-family="sans-serif">João Silva</text><text x="20" y="103" fill="#F59E0B" font-size="10" font-family="sans-serif">★★★★★</text><rect x="18" y="108" width="100" height="3" rx="1" fill="#333"/><rect x="18" y="115" width="80" height="3" rx="1" fill="#333"/><rect x="144" y="78" width="126" height="70" rx="8" fill="#1a1a1a" stroke="#333"/><text x="154" y="93" fill="#EEE" font-size="7" font-weight="bold" font-family="sans-serif">Maria R.</text><text x="154" y="103" fill="#F59E0B" font-size="10" font-family="sans-serif">★★★★</text></svg>`,

      // Comissões
      "comissoes-config": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="12" width="100" height="7" rx="3" fill="#EEE"/><rect x="64" y="30" width="186" height="110" rx="8" fill="#1a1a1a" stroke="#10B98144"/><text x="75" y="48" fill="#10B981" font-size="8" font-weight="bold" font-family="sans-serif">💸 Configurar Comissões</text><text x="75" y="64" fill="#666" font-size="7" font-family="sans-serif">COMISSÃO PADRÃO</text><rect x="75" y="68" width="80" height="14" rx="4" fill="#222" stroke="#10B981"/><text x="83" y="78" fill="#10B981" font-size="10" font-weight="bold" font-family="sans-serif">40%</text><text x="164" y="78" fill="#666" font-size="7" font-family="sans-serif">de cada serviço</text><text x="75" y="96" fill="#666" font-size="7" font-family="sans-serif">POR BARBEIRO</text><rect x="75" y="100" width="165" height="14" rx="4" fill="#222" stroke="#333"/><text x="83" y="110" fill="#EEE" font-size="7" font-family="sans-serif">Carlos Rocha — 45%</text><rect x="75" y="120" width="165" height="14" rx="4" fill="#222" stroke="#333"/><text x="83" y="130" fill="#EEE" font-size="7" font-family="sans-serif">Pedro Lima — 35%</text></svg>`,

      "comissoes-barbeiro": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111" opacity=".6"/><rect x="50" y="10" width="180" height="140" rx="12" fill="#1a1a1a" stroke="#10B981" stroke-width="1"/><text x="88" y="28" fill="#10B981" font-size="9" font-weight="bold" font-family="sans-serif">Carlos Rocha</text><text x="60" y="44" fill="#666" font-size="7" font-family="sans-serif">COMISSÃO PADRÃO</text><rect x="60" y="48" width="70" height="14" rx="4" fill="#10B98122" stroke="#10B981"/><text x="68" y="58" fill="#10B981" font-size="10" font-weight="bold" font-family="sans-serif">45%</text><text x="60" y="76" fill="#666" font-size="7" font-family="sans-serif">POR SERVIÇO (opcional)</text><rect x="60" y="80" width="160" height="14" rx="4" fill="#222" stroke="#333"/><text x="68" y="90" fill="#EEE" font-size="7" font-family="sans-serif">Corte de Cabelo — 50%</text><rect x="60" y="100" width="160" height="14" rx="4" fill="#222" stroke="#333"/><text x="68" y="110" fill="#EEE" font-size="7" font-family="sans-serif">Barba — 40%</text><rect x="60" y="120" width="160" height="14" rx="4" fill="#10B98112" stroke="#10B98133"/><text x="68" y="130" fill="#10B981" font-size="7" font-family="sans-serif">+ Adicionar exceção</text><rect x="80" y="142" width="120" height="10" rx="4" fill="#10B981"/><text x="118" y="149" fill="#FFF" font-size="7" font-weight="bold" font-family="sans-serif">Salvar</text></svg>`,

      "comissoes-calc": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="15" width="186" height="55" rx="8" fill="#1a1a1a" stroke="#333"/><rect x="72" y="22" width="60" height="5" rx="2" fill="#EEE"/><rect x="72" y="32" width="80" height="4" rx="2" fill="#555"/><rect x="160" y="18" width="80" height="18" rx="4" fill="#4ADE8022" stroke="#4ADE8044"/><text x="170" y="29" fill="#4ADE80" font-size="8" font-weight="bold" font-family="sans-serif">✓ Concluído</text><rect x="64" y="82" width="186" height="28" rx="6" fill="#0d1a0d" stroke="#10B98144"/><text x="78" y="94" fill="#10B981" font-size="8" font-weight="bold" font-family="sans-serif">+R$24,75 adicionado a Carlos</text><text x="72" y="104" fill="#10B981" font-size="7" font-family="sans-serif">Serviço: Corte + Barba (R$55 × 45%)</text><rect x="64" y="120" width="186" height="28" rx="6" fill="#1a1a1a" stroke="#333"/><rect x="72" y="128" width="80" height="5" rx="2" fill="#555"/><text x="210" y="136" fill="#10B981" font-size="8" font-weight="bold" font-family="sans-serif">+R$14</text></svg>`,

      "comissoes-view": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="12" width="110" height="7" rx="3" fill="#EEE"/><rect x="64" y="30" width="186" height="50" rx="8" fill="#0d1a0d" stroke="#10B98144"/><text x="75" y="46" fill="#666" font-size="7" font-family="sans-serif">MINHAS COMISSÕES — MAIO</text><text x="75" y="64" fill="#10B981" font-size="20" font-weight="bold" font-family="sans-serif">R$ 847,50</text><rect x="64" y="90" width="186" height="24" rx="6" fill="#1a1a1a" stroke="#333"/><rect x="72" y="98" width="60" height="4" rx="2" fill="#EEE"/><rect x="72" y="106" width="40" height="4" rx="2" fill="#555"/><text x="218" y="106" fill="#10B981" font-size="8" font-weight="bold" font-family="sans-serif">R$24,75</text><rect x="64" y="120" width="186" height="24" rx="6" fill="#1a1a1a" stroke="#333"/><rect x="72" y="128" width="70" height="4" rx="2" fill="#EEE"/><rect x="72" y="136" width="50" height="4" rx="2" fill="#555"/><text x="218" y="136" fill="#10B981" font-size="8" font-weight="bold" font-family="sans-serif">R$18,00</text></svg>`,

      // Configurações
      "config-menu": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="12" width="80" height="7" rx="3" fill="#EEE"/><rect x="64" y="30" width="186" height="22" rx="8" fill="#1a1a1a" stroke="#333"/><rect x="72" y="36" width="38" height="10" rx="4" fill="#6B728022" stroke="#6B7280"/><text x="77" y="43" fill="#6B7280" font-size="7" font-family="sans-serif">Dados</text><rect x="116" y="36" width="40" height="10" rx="4" fill="#333"/><text x="118" y="43" fill="#EEE" font-size="7" font-family="sans-serif">Horários</text><rect x="162" y="36" width="36" height="10" rx="4" fill="#333"/><text x="166" y="43" fill="#EEE" font-size="7" font-family="sans-serif">Equipe</text><rect x="204" y="36" width="40" height="10" rx="4" fill="#333"/><text x="206" y="43" fill="#EEE" font-size="7" font-family="sans-serif">Pagamentos</text><rect x="64" y="60" width="186" height="85" rx="8" fill="#1a1a1a" stroke="#6B728044"/><text x="75" y="76" fill="#666" font-size="7" font-family="sans-serif">NOME DA BARBEARIA</text><rect x="75" y="80" width="160" height="14" rx="4" fill="#222" stroke="#6B7280"/><text x="83" y="90" fill="#EEE" font-size="7" font-family="sans-serif">Mogiana Barber</text><text x="75" y="108" fill="#666" font-size="7" font-family="sans-serif">TELEFONE</text><rect x="75" y="112" width="160" height="14" rx="4" fill="#222" stroke="#333"/><text x="83" y="122" fill="#EEE" font-size="7" font-family="sans-serif">(16) 99999-9999</text><rect x="90" y="134" width="120" height="8" rx="3" fill="#6B7280"/><text x="125" y="140" fill="#FFF" font-size="6" font-weight="bold" font-family="sans-serif">Salvar</text></svg>`,

      "config-dados": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="20" width="186" height="120" rx="8" fill="#1a1a1a" stroke="#333"/><rect x="72" y="34" width="160" height="14" rx="4" fill="#222" stroke="#6B7280"/><text x="80" y="44" fill="#EEE" font-size="7" font-family="sans-serif">Mogiana Barber</text><rect x="72" y="54" width="160" height="14" rx="4" fill="#222" stroke="#333"/><text x="80" y="64" fill="#EEE" font-size="7" font-family="sans-serif">Rua das Barbearias, 123</text><rect x="72" y="74" width="76" height="14" rx="4" fill="#222" stroke="#333"/><text x="80" y="84" fill="#EEE" font-size="7" font-family="sans-serif">(16) 99999-0000</text><rect x="156" y="74" width="76" height="14" rx="4" fill="#222" stroke="#333"/><text x="164" y="84" fill="#555" font-size="7" font-family="sans-serif">@mogiana</text><rect x="72" y="94" width="160" height="14" rx="4" fill="#222" stroke="#333"/><text x="80" y="104" fill="#555" font-size="7" font-family="sans-serif">contato@mogiana.com.br</text><rect x="90" y="118" width="120" height="16" rx="6" fill="#6B7280"/><text x="127" y="128" fill="#FFF" font-size="7" font-weight="bold" font-family="sans-serif">Salvar dados</text></svg>`,

      "config-equipe": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="12" width="80" height="7" rx="3" fill="#EEE"/><rect x="200" y="10" width="55" height="11" rx="5" fill="#C9A84C"/><text x="206" y="18" fill="#0A0A0A" font-size="7" font-weight="bold" font-family="sans-serif">+ Barbeiro</text><rect x="64" y="30" width="186" height="30" rx="6" fill="#1a1a1a" stroke="#333"/><circle cx="84" cy="45" r="9" fill="#C9A84C22" stroke="#C9A84C44"/><text x="79" y="49" fill="#C9A84C" font-size="8" font-family="sans-serif">CR</text><rect x="100" y="39" width="60" height="5" rx="2" fill="#EEE"/><rect x="100" y="48" width="80" height="3" rx="1" fill="#555"/><rect x="64" y="66" width="186" height="30" rx="6" fill="#1a1a1a" stroke="#333"/><circle cx="84" cy="81" r="9" fill="#60A5FA22" stroke="#60A5FA44"/><text x="79" y="85" fill="#60A5FA" font-size="8" font-family="sans-serif">PL</text><rect x="100" y="75" width="55" height="5" rx="2" fill="#EEE"/><rect x="100" y="84" width="70" height="3" rx="1" fill="#555"/><rect x="64" y="102" width="186" height="30" rx="6" fill="#C9A84C12" stroke="#C9A84C33" stroke-dasharray="4"/><text x="140" y="120" fill="#C9A84C" font-size="18" font-family="sans-serif">+</text></svg>`,

      "config-pagamentos": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="15" width="186" height="120" rx="8" fill="#1a1a1a" stroke="#C9A84C44"/><text x="75" y="33" fill="#C9A84C" font-size="8" font-weight="bold" font-family="sans-serif">💳 Pagamentos Online (Asaas)</text><rect x="75" y="40" width="165" height="1" fill="#222"/><rect x="75" y="50" width="165" height="28" rx="6" fill="#0d2020" stroke="#4ADE8044"/><text x="85" y="62" fill="#4ADE80" font-size="8" font-weight="bold" font-family="sans-serif">✓ Conta Asaas conectada</text><text x="85" y="72" fill="#4ADE80" font-size="6" font-family="sans-serif">@mogiana · Saldo: R$ 1.240,00</text><rect x="75" y="86" width="80" height="20" rx="6" fill="#1E1E1E" stroke="#333"/><text x="83" y="98" fill="#EEE" font-size="7" font-family="sans-serif">⚡ Pix ✓</text><rect x="161" y="86" width="80" height="20" rx="6" fill="#1E1E1E" stroke="#333"/><text x="169" y="98" fill="#EEE" font-size="7" font-family="sans-serif">💳 Cartão ✓</text><rect x="75" y="112" width="165" height="16" rx="6" fill="#C9A84C"/><text x="115" y="122" fill="#0A0A0A" font-size="7" font-weight="bold" font-family="sans-serif">Ver extrato no Asaas</text></svg>`,

      // App
      "app-download": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="30" y="20" width="100" height="120" rx="12" fill="#1a1a1a" stroke="#3B82F6" stroke-width="1.5"/><rect x="40" y="35" width="80" height="45" rx="6" fill="#0d1525"/><text x="62" y="62" fill="#3B82F6" font-size="20" font-family="sans-serif">✂</text><text x="55" y="92" fill="#EEE" font-size="8" font-weight="bold" font-family="sans-serif">Barber Pro</text><rect x="44" y="100" width="72" height="14" rx="6" fill="#3B82F6"/><text x="60" y="109" fill="#FFF" font-size="7" font-weight="bold" font-family="sans-serif">App Store</text><rect x="44" y="120" width="72" height="14" rx="6" fill="#34A853"/><text x="56" y="129" fill="#FFF" font-size="7" font-weight="bold" font-family="sans-serif">Play Store</text><rect x="150" y="20" width="100" height="120" rx="12" fill="#1a1a1a" stroke="#34A853" stroke-width="1.5"/><rect x="160" y="35" width="80" height="45" rx="6" fill="#0d1a0d"/><text x="182" y="62" fill="#34A853" font-size="20" font-family="sans-serif">✂</text><text x="172" y="92" fill="#EEE" font-size="8" font-weight="bold" font-family="sans-serif">Barber Pro</text><rect x="164" y="100" width="72" height="14" rx="6" fill="#3B82F6"/><text x="180" y="109" fill="#FFF" font-size="7" font-weight="bold" font-family="sans-serif">App Store</text><rect x="164" y="120" width="72" height="14" rx="6" fill="#34A853"/><text x="176" y="129" fill="#FFF" font-size="7" font-weight="bold" font-family="sans-serif">Play Store</text></svg>`,

      "app-login": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="80" y="10" width="120" height="140" rx="16" fill="#1a1a1a" stroke="#3B82F644"/><text x="117" y="38" fill="#C9A84C" font-size="11" font-weight="bold" font-family="sans-serif">✂ Barber</text><text x="120" y="50" fill="#C9A84C" font-size="9" font-weight="bold" font-family="sans-serif">Pro</text><rect x="92" y="62" width="96" height="16" rx="6" fill="#222" stroke="#3B82F644"/><text x="100" y="73" fill="#555" font-size="7" font-family="sans-serif">e-mail...</text><rect x="92" y="84" width="96" height="16" rx="6" fill="#222" stroke="#333"/><text x="100" y="95" fill="#555" font-size="7" font-family="sans-serif">senha...</text><rect x="95" y="108" width="90" height="16" rx="8" fill="#3B82F6"/><text x="120" y="119" fill="#FFF" font-size="7" font-weight="bold" font-family="sans-serif">Entrar</text><rect x="95" y="130" width="90" height="14" rx="6" fill="#C9A84C22" stroke="#C9A84C44"/><text x="107" y="139" fill="#C9A84C" font-size="7" font-family="sans-serif">Google Login</text></svg>`,

      "app-agenda": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="70" y="8" width="140" height="144" rx="16" fill="#1a1a1a" stroke="#3B82F644"/><rect x="80" y="20" width="120" height="14" rx="4" fill="#0d1525"/><text x="100" y="30" fill="#3B82F6" font-size="8" font-weight="bold" font-family="sans-serif">📅 Agenda — Hoje</text><rect x="80" y="40" width="120" height="22" rx="6" fill="#0d1525" stroke="#C9A84C44"/><rect x="86" y="46" width="40" height="4" rx="2" fill="#EEE"/><rect x="86" y="54" width="60" height="3" rx="1" fill="#555"/><rect x="160" y="44" width="34" height="12" rx="4" fill="#4ADE8022"/><text x="165" y="52" fill="#4ADE80" font-size="6" font-family="sans-serif">✓ Conf.</text><rect x="80" y="68" width="120" height="22" rx="6" fill="#0d1a25" stroke="#2196F344"/><rect x="86" y="74" width="40" height="4" rx="2" fill="#EEE"/><rect x="86" y="82" width="55" height="3" rx="1" fill="#555"/><rect x="160" y="72" width="34" height="12" rx="4" fill="#2196F322"/><text x="163" y="80" fill="#2196F3" font-size="6" font-family="sans-serif">Em and.</text><rect x="80" y="96" width="120" height="22" rx="6" fill="#1a1a1a" stroke="#333"/><rect x="86" y="102" width="50" height="4" rx="2" fill="#EEE"/><rect x="86" y="110" width="40" height="3" rx="1" fill="#555"/><rect x="80" y="130" width="28" height="18" rx="6" fill="#1E1E1E" stroke="#333"/><rect x="114" y="130" width="28" height="18" rx="6" fill="#1E1E1E" stroke="#333"/><rect x="148" y="130" width="28" height="18" rx="6" fill="#1E1E1E" stroke="#333"/><text x="87" y="141" fill="#EEE" font-size="8" font-family="sans-serif">📅</text><text x="121" y="141" fill="#EEE" font-size="8" font-family="sans-serif">👥</text><text x="155" y="141" fill="#EEE" font-size="8" font-family="sans-serif">💰</text></svg>`,

      "app-notif": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="70" y="8" width="140" height="144" rx="16" fill="#1a1a1a" stroke="#3B82F644"/><rect x="80" y="20" width="120" height="80" rx="6" fill="#0d1525"/><rect x="86" y="28" width="108" height="28" rx="6" fill="#1E2D40" stroke="#3B82F644"/><rect x="90" y="34" width="16" height="16" rx="4" fill="#C9A84C22" stroke="#C9A84C44"/><text x="93" y="45" fill="#C9A84C" font-size="9" font-family="sans-serif">✂</text><rect x="112" y="36" width="76" height="4" rx="2" fill="#EEE"/><rect x="112" y="44" width="60" height="3" rx="1" fill="#555"/><rect x="86" y="62" width="108" height="28" rx="6" fill="#1E2D40" stroke="#C9A84C44"/><rect x="90" y="68" width="16" height="16" rx="4" fill="#C9A84C22" stroke="#C9A84C44"/><text x="93" y="79" fill="#C9A84C" font-size="9" font-family="sans-serif">✂</text><rect x="112" y="70" width="76" height="4" rx="2" fill="#EEE"/><rect x="112" y="78" width="50" height="3" rx="1" fill="#555"/><rect x="80" y="108" width="120" height="30" rx="8" fill="#0d2020" stroke="#4ADE8044"/><text x="100" y="120" fill="#4ADE80" font-size="7" font-weight="bold" font-family="sans-serif">🔔 Ativar notificações</text><text x="88" y="132" fill="#4ADE80" font-size="6" font-family="sans-serif">Nunca perca um agendamento!</text></svg>`,


      "planos-list": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="12" width="80" height="7" rx="3" fill="#EEE"/><rect x="194" y="10" width="60" height="11" rx="5" fill="#F59E0B"/><text x="205" y="19" fill="#0A0A0A" font-size="7" font-weight="bold" font-family="sans-serif">+ Novo Plano</text><rect x="64" y="32" width="58" height="110" rx="8" fill="#1a1a1a" stroke="#F59E0B" stroke-width="1.5"/><text x="76" y="50" fill="#F59E0B" font-size="8" font-weight="bold" font-family="sans-serif">Solo</text><text x="69" y="66" fill="#EEE" font-size="11" font-weight="bold" font-family="sans-serif">R$49</text><text x="72" y="76" fill="#666" font-size="6" font-family="sans-serif">/mês</text><rect x="70" y="82" width="46" height="4" rx="2" fill="#333"/><rect x="70" y="90" width="38" height="4" rx="2" fill="#333"/><rect x="70" y="98" width="42" height="4" rx="2" fill="#333"/><rect x="70" y="128" width="46" height="10" rx="4" fill="#F59E0B"/><text x="82" y="135" fill="#0A0A0A" font-size="7" font-weight="bold" font-family="sans-serif">Editar</text><rect x="130" y="32" width="58" height="110" rx="8" fill="#1a1a1a" stroke="#333"/><text x="142" y="50" fill="#EEE" font-size="8" font-weight="bold" font-family="sans-serif">Equipe</text><text x="134" y="66" fill="#EEE" font-size="11" font-weight="bold" font-family="sans-serif">R$89</text><rect x="196" y="32" width="58" height="110" rx="8" fill="#1a1a1a" stroke="#333"/><text x="204" y="50" fill="#EEE" font-size="8" font-weight="bold" font-family="sans-serif">Estúdio</text><text x="198" y="66" fill="#EEE" font-size="11" font-weight="bold" font-family="sans-serif">R$149</text></svg>`,

      "plano-novo": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111" opacity=".5"/><rect x="50" y="10" width="180" height="140" rx="12" fill="#1a1a1a" stroke="#F59E0B" stroke-width="1"/><text x="100" y="28" fill="#EEE" font-size="9" font-weight="bold" font-family="sans-serif">Novo Plano</text><text x="60" y="44" fill="#666" font-size="7" font-family="sans-serif">NOME DO PLANO</text><rect x="60" y="48" width="160" height="14" rx="4" fill="#222" stroke="#F59E0B" stroke-width="1"/><text x="68" y="58" fill="#EEE" font-size="7" font-family="sans-serif">Plano Premium</text><text x="60" y="74" fill="#666" font-size="7" font-family="sans-serif">PREÇO MENSAL</text><rect x="60" y="78" width="75" height="14" rx="4" fill="#222" stroke="#333"/><text x="68" y="88" fill="#C9A84C" font-size="8" font-weight="bold" font-family="sans-serif">R$ 89,00</text><text x="60" y="105" fill="#666" font-size="7" font-family="sans-serif">DESCRIÇÃO</text><rect x="60" y="109" width="160" height="22" rx="4" fill="#222" stroke="#333"/><text x="68" y="122" fill="#555" font-size="7" font-family="sans-serif">Inclui 4 cortes e 4 barbas...</text><rect x="75" y="138" width="130" height="10" rx="4" fill="#F59E0B"/><text x="116" y="145" fill="#0A0A0A" font-size="7" font-weight="bold" font-family="sans-serif">Criar Plano</text></svg>`,

      "plano-servicos": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111" opacity=".5"/><rect x="50" y="10" width="180" height="140" rx="12" fill="#1a1a1a" stroke="#2a2a2a"/><text x="88" y="28" fill="#EEE" font-size="9" font-weight="bold" font-family="sans-serif">Serviços do Plano</text><rect x="60" y="35" width="160" height="20" rx="6" fill="#1a1505" stroke="#F59E0B44"/><text x="68" y="45" fill="#EEE" font-size="8" font-family="sans-serif">✓ Corte de Cabelo</text><text x="186" y="45" fill="#F59E0B" font-size="8" font-family="sans-serif">4x/mês</text><rect x="60" y="60" width="160" height="20" rx="6" fill="#1a1505" stroke="#F59E0B44"/><text x="68" y="70" fill="#EEE" font-size="8" font-family="sans-serif">✓ Barba</text><text x="186" y="70" fill="#F59E0B" font-size="8" font-family="sans-serif">4x/mês</text><rect x="60" y="85" width="160" height="20" rx="6" fill="#1a1a1a" stroke="#333"/><text x="68" y="95" fill="#555" font-size="8" font-family="sans-serif">○ Hidratação</text><text x="186" y="95" fill="#555" font-size="8" font-family="sans-serif">—</text><rect x="60" y="110" width="160" height="20" rx="6" fill="#1a1a1a" stroke="#333"/><text x="68" y="120" fill="#555" font-size="8" font-family="sans-serif">○ Sobrancelha</text><text x="186" y="120" fill="#555" font-size="8" font-family="sans-serif">—</text><rect x="75" y="138" width="130" height="10" rx="4" fill="#F59E0B"/><text x="112" y="145" fill="#0A0A0A" font-size="7" font-weight="bold" font-family="sans-serif">Salvar</text></svg>`,

      "plano-pub": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#0f0f1a"/><rect x="10" y="10" width="260" height="18" rx="4" fill="#1a1a2e"/><text x="85" y="22" fill="#EEE" font-size="8" font-weight="bold" font-family="sans-serif">✂ Mogiana — Assinaturas</text><rect x="10" y="35" width="80" height="110" rx="8" fill="#1a1505" stroke="#F59E0B" stroke-width="1.5"/><text x="26" y="52" fill="#F59E0B" font-size="8" font-weight="bold" font-family="sans-serif">Premium</text><text x="20" y="68" fill="#EEE" font-size="14" font-weight="bold" font-family="sans-serif">R$89</text><text x="22" y="77" fill="#F59E0B" font-size="6" font-family="sans-serif">/mês</text><rect x="16" y="82" width="68" height="3" rx="1" fill="#333"/><rect x="16" y="90" width="55" height="3" rx="1" fill="#333"/><rect x="16" y="98" width="62" height="3" rx="1" fill="#333"/><rect x="16" y="120" width="68" height="16" rx="6" fill="#F59E0B"/><text x="29" y="130" fill="#0A0A0A" font-size="7" font-weight="bold" font-family="sans-serif">Assinar</text><rect x="100" y="35" width="80" height="110" rx="8" fill="#1a1a1a" stroke="#333"/><rect x="190" y="35" width="80" height="110" rx="8" fill="#1a1a1a" stroke="#333"/></svg>`,

      "produtos-list": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="12" width="80" height="7" rx="3" fill="#EEE"/><rect x="194" y="10" width="60" height="11" rx="5" fill="#EC4899"/><text x="200" y="19" fill="#FFF" font-size="7" font-weight="bold" font-family="sans-serif">+ Produto</text><rect x="64" y="32" width="58" height="80" rx="8" fill="#1a1a1a" stroke="#333"/><rect x="68" y="36" width="50" height="35" rx="4" fill="#EC489922"/><text x="84" y="57" fill="#EC4899" font-size="20" font-family="sans-serif">🧴</text><rect x="68" y="76" width="50" height="5" rx="2" fill="#EEE"/><rect x="68" y="85" width="35" height="4" rx="2" fill="#C9A84C"/><rect x="68" y="93" width="25" height="4" rx="2" fill="#555"/><rect x="130" y="32" width="58" height="80" rx="8" fill="#1a1a1a" stroke="#333"/><rect x="134" y="36" width="50" height="35" rx="4" fill="#60A5FA22"/><text x="150" y="57" fill="#60A5FA" font-size="20" font-family="sans-serif">💈</text><rect x="134" y="76" width="50" height="5" rx="2" fill="#EEE"/><rect x="134" y="85" width="35" height="4" rx="2" fill="#C9A84C"/><rect x="196" y="32" width="58" height="80" rx="8" fill="#1a1a1a" stroke="#EC4899" stroke-dasharray="3"/><text x="216" y="75" fill="#EC4899" font-size="24" font-family="sans-serif">+</text></svg>`,

      "produto-novo": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111" opacity=".5"/><rect x="50" y="10" width="180" height="140" rx="12" fill="#1a1a1a" stroke="#EC4899" stroke-width="1"/><text x="95" y="28" fill="#EEE" font-size="9" font-weight="bold" font-family="sans-serif">Novo Produto</text><text x="60" y="44" fill="#666" font-size="7" font-family="sans-serif">NOME</text><rect x="60" y="48" width="160" height="14" rx="4" fill="#222" stroke="#EC4899" stroke-width="1"/><text x="68" y="58" fill="#EEE" font-size="7" font-family="sans-serif">Pomada Baboon Black</text><text x="60" y="74" fill="#666" font-size="7" font-family="sans-serif">PREÇO &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; ESTOQUE</text><rect x="60" y="78" width="72" height="14" rx="4" fill="#222" stroke="#333"/><text x="68" y="88" fill="#C9A84C" font-size="8" font-weight="bold" font-family="sans-serif">R$ 50,00</text><rect x="140" y="78" width="80" height="14" rx="4" fill="#222" stroke="#333"/><text x="148" y="88" fill="#EEE" font-size="7" font-family="sans-serif">25 unidades</text><rect x="60" y="98" width="160" height="28" rx="4" fill="#222" stroke="#333" stroke-dasharray="3"/><text x="105" y="115" fill="#555" font-size="8" font-family="sans-serif">📷 Foto do produto</text><rect x="75" y="134" width="130" height="12" rx="5" fill="#EC4899"/><text x="111" y="142" fill="#FFF" font-size="7" font-weight="bold" font-family="sans-serif">Cadastrar Produto</text></svg>`,

      "estoque-dash": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="12" width="80" height="7" rx="3" fill="#EEE"/><rect x="64" y="32" width="186" height="24" rx="6" fill="#1a0808" stroke="#F8717144"/><text x="72" y="44" fill="#F87171" font-size="8" font-weight="bold" font-family="sans-serif">⚠ 2 produtos abaixo do mínimo</text><rect x="64" y="62" width="186" height="22" rx="6" fill="#1a1a1a" stroke="#333"/><rect x="70" y="68" width="40" height="4" rx="2" fill="#F87171"/><rect x="118" y="68" width="50" height="4" rx="2" fill="#EEE"/><text x="210" y="72" fill="#F87171" font-size="8" font-family="sans-serif">2/10</text><rect x="70" y="76" width="4" height="4" rx="1" fill="#F87171"/><text x="78" y="80" fill="#555" font-size="6" font-family="sans-serif">Estoque crítico</text><rect x="64" y="90" width="186" height="22" rx="6" fill="#1a1a1a" stroke="#333"/><rect x="70" y="96" width="55" height="4" rx="2" fill="#4ADE80"/><rect x="133" y="96" width="50" height="4" rx="2" fill="#EEE"/><text x="210" y="100" fill="#4ADE80" font-size="8" font-family="sans-serif">25/10</text><rect x="64" y="118" width="186" height="22" rx="6" fill="#1a1a1a" stroke="#333"/><rect x="70" y="124" width="35" height="4" rx="2" fill="#FBBF24"/><rect x="113" y="124" width="50" height="4" rx="2" fill="#EEE"/><text x="210" y="128" fill="#FBBF24" font-size="8" font-family="sans-serif">5/10</text></svg>`,

      "produto-pub": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#0f0f1a"/><rect x="10" y="8" width="260" height="18" rx="4" fill="#1a1a2e"/><text x="90" y="20" fill="#EEE" font-size="8" font-weight="bold" font-family="sans-serif">✂ Mogiana — Produtos</text><rect x="10" y="32" width="130" height="120" rx="8" fill="#1a1a1a" stroke="#EC4899" stroke-width="1"/><rect x="16" y="38" width="118" height="60" rx="4" fill="#EC489922"/><text x="60" y="72" fill="#EC4899" font-size="32" font-family="sans-serif">🧴</text><rect x="16" y="104" width="80" height="6" rx="2" fill="#EEE"/><rect x="16" y="115" width="50" height="5" rx="2" fill="#C9A84C"/><rect x="16" y="126" width="118" height="18" rx="6" fill="#EC4899"/><text x="42" y="137" fill="#FFF" font-size="8" font-weight="bold" font-family="sans-serif">🛒 Adicionar</text><rect x="148" y="32" width="122" height="120" rx="8" fill="#1a1a1a" stroke="#333"/><rect x="154" y="38" width="110" height="60" rx="4" fill="#60A5FA22"/><text x="196" y="72" fill="#60A5FA" font-size="32" font-family="sans-serif">💈</text></svg>`,

      "relatorios-dash": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="12" width="100" height="7" rx="3" fill="#EEE"/><rect x="64" y="30" width="56" height="44" rx="8" fill="#0d2818" stroke="#4ADE8044"/><text x="72" y="46" fill="#4ADE80" font-size="6" font-family="sans-serif">ATENDIMENTOS</text><text x="69" y="62" fill="#4ADE80" font-size="14" font-weight="bold" font-family="sans-serif">127</text><rect x="128" y="30" width="56" height="44" rx="8" fill="#1a1505" stroke="#C9A84C44"/><text x="136" y="46" fill="#C9A84C" font-size="6" font-family="sans-serif">RECEITA TOTAL</text><text x="132" y="62" fill="#C9A84C" font-size="11" font-weight="bold" font-family="sans-serif">R$6.985</text><rect x="192" y="30" width="58" height="44" rx="8" fill="#1a1a2e" stroke="#60A5FA44"/><text x="200" y="46" fill="#60A5FA" font-size="6" font-family="sans-serif">TICKET MÉDIO</text><text x="197" y="62" fill="#60A5FA" font-size="14" font-weight="bold" font-family="sans-serif">R$55</text><rect x="64" y="82" width="186" height="65" rx="8" fill="#1a1a1a" stroke="#333"/><text x="72" y="96" fill="#666" font-size="7" font-family="sans-serif">Serviços mais vendidos</text><rect x="72" y="102" width="120" height="6" rx="3" fill="#C9A84C"/><text x="197" y="108" fill="#C9A84C" font-size="7" font-family="sans-serif">48</text><rect x="72" y="114" width="90" height="6" rx="3" fill="#60A5FA"/><text x="167" y="120" fill="#60A5FA" font-size="7" font-family="sans-serif">35</text><rect x="72" y="126" width="60" height="6" rx="3" fill="#A78BFA"/><text x="137" y="132" fill="#A78BFA" font-size="7" font-family="sans-serif">22</text></svg>`,

      "relatorios-charts": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="12" width="186" height="130" rx="8" fill="#1a1a1a" stroke="#333"/><text x="72" y="28" fill="#EEE" font-size="8" font-weight="bold" font-family="sans-serif">Receita por barbeiro</text><rect x="72" y="38" width="8" height="8" rx="2" fill="#C9A84C"/><text x="84" y="45" fill="#666" font-size="7" font-family="sans-serif">Carlos — R$3.200</text><rect x="72" y="50" width="8" height="8" rx="2" fill="#60A5FA"/><text x="84" y="57" fill="#666" font-size="7" font-family="sans-serif">Rafael — R$2.100</text><rect x="72" y="62" width="8" height="8" rx="2" fill="#4ADE80"/><text x="84" y="69" fill="#666" font-size="7" font-family="sans-serif">Marcos — R$1.685</text><rect x="72" y="80" width="100" height="55" rx="6" fill="#0A0A0A"/><rect x="78" y="120" width="14" height="10" rx="2" fill="#C9A84C"/><rect x="98" y="110" width="14" height="20" rx="2" fill="#C9A84C" opacity=".8"/><rect x="118" y="100" width="14" height="30" rx="2" fill="#C9A84C" opacity=".6"/><rect x="138" y="95" width="14" height="35" rx="2" fill="#C9A84C" opacity=".4"/><rect x="158" y="108" width="14" height="22" rx="2" fill="#C9A84C" opacity=".6"/><rect x="180" y="88" width="60" height="47" rx="30" fill="#C9A84C22" stroke="#C9A84C44"/><text x="199" y="115" fill="#C9A84C" font-size="16" font-weight="bold" font-family="sans-serif">R$</text></svg>`,

      "comissoes-config": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="12" width="80" height="7" rx="3" fill="#EEE"/><rect x="64" y="32" width="186" height="22" rx="6" fill="#1a1a1a" stroke="#333"/><text x="72" y="44" fill="#EEE" font-size="8" font-family="sans-serif">Carlos Rocha</text><rect x="190" y="35" width="55" height="14" rx="4" fill="#C9A84C22" stroke="#C9A84C"/><text x="202" y="44" fill="#C9A84C" font-size="8" font-weight="bold" font-family="sans-serif">30%</text><rect x="64" y="60" width="186" height="22" rx="6" fill="#1a1a1a" stroke="#333"/><text x="72" y="72" fill="#EEE" font-size="8" font-family="sans-serif">Rafael Silva</text><rect x="190" y="63" width="55" height="14" rx="4" fill="#C9A84C22" stroke="#C9A84C"/><text x="202" y="72" fill="#C9A84C" font-size="8" font-weight="bold" font-family="sans-serif">25%</text><rect x="64" y="88" width="186" height="22" rx="6" fill="#1a1a1a" stroke="#333"/><text x="72" y="100" fill="#EEE" font-size="8" font-family="sans-serif">Marcos Lima</text><rect x="190" y="91" width="55" height="14" rx="4" fill="#C9A84C22" stroke="#C9A84C"/><text x="202" y="100" fill="#C9A84C" font-size="8" font-weight="bold" font-family="sans-serif">28%</text><rect x="64" y="118" width="186" height="28" rx="6" fill="#0d2818" stroke="#4ADE8044"/><text x="88" y="128" fill="#4ADE80" font-size="7" font-family="sans-serif">Total em comissões este mês:</text><text x="88" y="139" fill="#4ADE80" font-size="9" font-weight="bold" font-family="sans-serif">R$ 2.095,50</text></svg>`,

      "fidelidade-config": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="12" width="80" height="7" rx="3" fill="#EEE"/><rect x="64" y="30" width="186" height="55" rx="8" fill="#1a1a1a" stroke="#EAB30844"/><text x="72" y="46" fill="#EAB308" font-size="8" font-weight="bold" font-family="sans-serif">⭐ Programa de Pontos</text><text x="72" y="60" fill="#666" font-size="7" font-family="sans-serif">Pontos por Corte</text><rect x="72" y="64" width="50" height="14" rx="4" fill="#222" stroke="#EAB308"/><text x="80" y="74" fill="#EAB308" font-size="9" font-weight="bold" font-family="sans-serif">10 pts</text><text x="130" y="60" fill="#666" font-size="7" font-family="sans-serif">Pontos por Barba</text><rect x="130" y="64" width="50" height="14" rx="4" fill="#222" stroke="#333"/><text x="138" y="74" fill="#EEE" font-size="9" font-weight="bold" font-family="sans-serif">5 pts</text><rect x="64" y="92" width="186" height="20" rx="6" fill="#1a1a1a" stroke="#333"/><text x="72" y="104" fill="#EEE" font-size="8" font-family="sans-serif">🎁 Recompensa: 100 pts = Corte grátis</text><rect x="64" y="118" width="186" height="20" rx="6" fill="#1a1a1a" stroke="#333"/><text x="72" y="130" fill="#EEE" font-size="8" font-family="sans-serif">🎁 Recompensa: 200 pts = Kit presente</text></svg>`,

      "fidelidade-pontos": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="15" width="186" height="60" rx="8" fill="#1a1a1a" stroke="#333"/><circle cx="90" cy="45" r="18" fill="#C9A84C22" stroke="#C9A84C44"/><text x="83" y="49" fill="#C9A84C" font-size="11" font-weight="bold" font-family="sans-serif">JS</text><rect x="116" y="28" width="80" height="6" rx="2" fill="#EEE"/><rect x="116" y="40" width="60" height="4" rx="2" fill="#555"/><rect x="116" y="52" width="100" height="14" rx="4" fill="#EAB30822" stroke="#EAB30844"/><text x="124" y="62" fill="#EAB308" font-size="8" font-weight="bold" font-family="sans-serif">⭐ 85 pontos</text><rect x="64" y="85" width="186" height="18" rx="6" fill="#0d2818" stroke="#4ADE8044"/><text x="84" y="96" fill="#4ADE80" font-size="8" font-family="sans-serif">✓ +10 pts adicionados após o atendimento</text><rect x="64" y="110" width="186" height="40" rx="8" fill="#1a1a1a" stroke="#EAB30844"/><text x="72" y="126" fill="#EAB308" font-size="8" font-weight="bold" font-family="sans-serif">Progresso para recompensa:</text><rect x="72" y="132" width="150" height="8" rx="4" fill="#222"/><rect x="72" y="132" width="127" height="8" rx="4" fill="#EAB308"/><text x="228" y="139" fill="#EAB308" font-size="7" font-family="sans-serif">85%</text></svg>`,

      "avaliacoes-dash": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="12" width="80" height="7" rx="3" fill="#EEE"/><rect x="64" y="30" width="80" height="55" rx="8" fill="#1a1a1a" stroke="#EAB30844"/><text x="80" y="52" fill="#EAB308" font-size="28" font-weight="bold" font-family="sans-serif">4.9</text><text x="72" y="66" fill="#EAB308" font-size="12" font-family="sans-serif">★★★★★</text><text x="76" y="78" fill="#666" font-size="7" font-family="sans-serif">127 avaliações</text><rect x="152" y="30" width="98" height="55" rx="8" fill="#1a1a1a" stroke="#333"/><rect x="160" y="38" width="5" height="5" rx="1" fill="#EAB308"/><text x="170" y="43" fill="#666" font-size="7" font-family="sans-serif">5 ★</text><rect x="192" y="39" width="50" height="4" rx="2" fill="#EAB308" opacity=".8"/><text x="246" y="43" fill="#666" font-size="6" font-family="sans-serif">89</text><rect x="160" y="50" width="5" height="5" rx="1" fill="#EAB308" opacity=".6"/><text x="170" y="55" fill="#666" font-size="7" font-family="sans-serif">4 ★</text><rect x="192" y="51" width="25" height="4" rx="2" fill="#EAB308" opacity=".5"/><text x="221" y="55" fill="#666" font-size="6" font-family="sans-serif">28</text><rect x="64" y="92" width="186" height="30" rx="8" fill="#1a1a1a" stroke="#333"/><text x="72" y="104" fill="#EAB308" font-size="8" font-family="sans-serif">★★★★★</text><text x="110" y="104" fill="#EEE" font-size="7" font-family="sans-serif">João S. — "Melhor barbearia..."</text><text x="72" y="116" fill="#555" font-size="7" font-family="sans-serif">há 2 dias</text><rect x="64" y="128" width="186" height="20" rx="6" fill="#1a1a1a" stroke="#333"/></svg>`,

      "avaliacoes-whats": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="20" width="186" height="50" rx="8" fill="#1a1a1a" stroke="#333"/><rect x="70" y="28" width="55" height="5" rx="2" fill="#EEE"/><rect x="70" y="38" width="80" height="4" rx="2" fill="#555"/><rect x="70" y="48" width="55" height="5" rx="2" fill="#4ADE80"/><rect x="180" y="28" width="60" height="30" rx="6" fill="#C9A84C"/><text x="190" y="41" fill="#0A0A0A" font-size="7" font-weight="bold" font-family="sans-serif">✓ Concluído</text><rect x="64" y="80" width="186" height="60" rx="8" fill="#0d2015" stroke="#25D36644"/><text x="72" y="96" fill="#25D366" font-size="8" font-weight="bold" font-family="sans-serif">💬 WhatsApp enviado automaticamente</text><text x="72" y="110" fill="#555" font-size="7" font-family="sans-serif">Para: João Silva — (16) 99272-2728</text><rect x="72" y="118" width="170" height="16" rx="4" fill="#1a1a1a" stroke="#333"/><text x="80" y="128" fill="#555" font-size="7" font-family="sans-serif">"Como foi seu atendimento? ⭐ Avalie..."</text></svg>`,

      "retorno-config": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="12" width="100" height="7" rx="3" fill="#EEE"/><rect x="64" y="30" width="186" height="50" rx="8" fill="#1a1a1a" stroke="#8B5CF644"/><text x="72" y="46" fill="#8B5CF6" font-size="8" font-weight="bold" font-family="sans-serif">⏰ Retorno Automático</text><text x="72" y="60" fill="#666" font-size="7" font-family="sans-serif">Enviar mensagem após</text><rect x="72" y="64" width="50" height="14" rx="4" fill="#222" stroke="#8B5CF6"/><text x="80" y="74" fill="#8B5CF6" font-size="9" font-weight="bold" font-family="sans-serif">30 dias</text><text x="130" y="72" fill="#666" font-size="7" font-family="sans-serif">sem visita</text><rect x="64" y="88" width="186" height="40" rx="8" fill="#1a1a1a" stroke="#333"/><text x="72" y="100" fill="#666" font-size="7" font-family="sans-serif">MENSAGEM PERSONALIZADA</text><rect x="72" y="104" width="170" height="18" rx="4" fill="#222"/><text x="80" y="115" fill="#555" font-size="7" font-family="sans-serif">"Olá {nome}! Faz um tempo que não te vemos..."</text><rect x="64" y="135" width="90" height="14" rx="6" fill="#8B5CF6"/><text x="78" y="144" fill="#FFF" font-size="7" font-weight="bold" font-family="sans-serif">Ativar retorno</text></svg>`,

      "promo-nova": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111" opacity=".5"/><rect x="50" y="10" width="180" height="140" rx="12" fill="#1a1a1a" stroke="#8B5CF6" stroke-width="1"/><text x="98" y="28" fill="#EEE" font-size="9" font-weight="bold" font-family="sans-serif">Nova Promoção</text><text x="60" y="44" fill="#666" font-size="7" font-family="sans-serif">CÓDIGO DO CUPOM</text><rect x="60" y="48" width="160" height="14" rx="4" fill="#222" stroke="#8B5CF6" stroke-width="1"/><text x="68" y="58" fill="#8B5CF6" font-size="9" font-weight="bold" font-family="sans-serif">PROMO20</text><text x="60" y="74" fill="#666" font-size="7" font-family="sans-serif">DESCONTO &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; TIPO</text><rect x="60" y="78" width="65" height="14" rx="4" fill="#222" stroke="#333"/><text x="68" y="88" fill="#EEE" font-size="8" font-family="sans-serif">20</text><rect x="133" y="78" width="87" height="14" rx="4" fill="#8B5CF622" stroke="#8B5CF6"/><text x="141" y="88" fill="#8B5CF6" font-size="7" font-weight="bold" font-family="sans-serif">% Percentual</text><text x="60" y="106" fill="#666" font-size="7" font-family="sans-serif">VALIDADE</text><rect x="60" y="110" width="160" height="14" rx="4" fill="#222" stroke="#333"/><text x="68" y="120" fill="#EEE" font-size="7" font-family="sans-serif">01/06/2026 até 30/06/2026</text><rect x="75" y="134" width="130" height="12" rx="5" fill="#8B5CF6"/><text x="112" y="142" fill="#FFF" font-size="7" font-weight="bold" font-family="sans-serif">Criar Cupom</text></svg>`,

      "promo-cupom": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="20" width="186" height="60" rx="8" fill="#1a0a3d" stroke="#8B5CF644"/><text x="110" y="40" fill="#8B5CF6" font-size="20" font-weight="bold" font-family="sans-serif">PROMO20</text><text x="90" y="55" fill="#8B5CF6" font-size="9" font-family="sans-serif">20% de desconto · Válido até 30/06</text><text x="82" y="68" fill="#555" font-size="8" font-family="sans-serif">Compartilhe este código com seus clientes</text><rect x="64" y="90" width="88" height="35" rx="8" fill="#1a1a1a" stroke="#25D366"/><text x="72" y="106" fill="#25D366" font-size="8" font-family="sans-serif">💬 WhatsApp</text><text x="70" y="117" fill="#555" font-size="6" font-family="sans-serif">Enviar para clientes</text><rect x="162" y="90" width="88" height="35" rx="8" fill="#1a1a1a" stroke="#C9A84C"/><text x="182" y="106" fill="#C9A84C" font-size="8" font-family="sans-serif">📋 Copiar link</text><text x="170" y="117" fill="#555" font-size="6" font-family="sans-serif">Colar no Instagram</text></svg>`,

      "promo-resultado": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="12" width="100" height="7" rx="3" fill="#EEE"/><rect x="64" y="30" width="58" height="44" rx="8" fill="#1a1a1a" stroke="#4ADE8044"/><text x="72" y="46" fill="#4ADE80" font-size="6" font-family="sans-serif">CUPONS USADOS</text><text x="78" y="62" fill="#4ADE80" font-size="18" font-weight="bold" font-family="sans-serif">23</text><rect x="130" y="30" width="58" height="44" rx="8" fill="#1a1a1a" stroke="#C9A84C44"/><text x="138" y="46" fill="#C9A84C" font-size="6" font-family="sans-serif">RECEITA GERADA</text><text x="133" y="62" fill="#C9A84C" font-size="11" font-weight="bold" font-family="sans-serif">R$1.265</text><rect x="196" y="30" width="54" height="44" rx="8" fill="#1a1a1a" stroke="#60A5FA44"/><text x="204" y="46" fill="#60A5FA" font-size="6" font-family="sans-serif">CONVERSÃO</text><text x="204" y="62" fill="#60A5FA" font-size="18" font-weight="bold" font-family="sans-serif">76%</text><rect x="64" y="82" width="186" height="65" rx="8" fill="#1a1a1a" stroke="#333"/><text x="72" y="98" fill="#666" font-size="7" font-family="sans-serif">PROMO20 — 20% desconto</text><rect x="72" y="104" width="140" height="5" rx="2" fill="#8B5CF6" opacity=".7"/><text x="218" y="109" fill="#8B5CF6" font-size="7" font-family="sans-serif">23/30</text><text x="72" y="122" fill="#666" font-size="7" font-family="sans-serif">BEMVINDO — R$10 desconto</text><rect x="72" y="128" width="60" height="5" rx="2" fill="#8B5CF6" opacity=".4"/><text x="138" y="133" fill="#555" font-size="7" font-family="sans-serif">8/30</text></svg>`,

      "config-geral": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="12" width="80" height="7" rx="3" fill="#EEE"/><rect x="64" y="30" width="186" height="18" rx="6" fill="#1a1a1a" stroke="#6B728044"/><text x="78" y="42" fill="#6B7280" font-size="8" font-weight="bold" font-family="sans-serif">⚙ Geral</text><rect x="64" y="54" width="186" height="18" rx="6" fill="#6B728022" stroke="#6B7280"/><text x="78" y="66" fill="#EEE" font-size="8" font-weight="bold" font-family="sans-serif">💳 Pagamentos Online</text><rect x="64" y="78" width="186" height="18" rx="6" fill="#1a1a1a" stroke="#6B728044"/><text x="78" y="90" fill="#6B7280" font-size="8" font-weight="bold" font-family="sans-serif">💬 WhatsApp</text><rect x="64" y="102" width="186" height="18" rx="6" fill="#1a1a1a" stroke="#6B728044"/><text x="78" y="114" fill="#6B7280" font-size="8" font-weight="bold" font-family="sans-serif">👥 Barbeiros</text><rect x="64" y="126" width="186" height="18" rx="6" fill="#1a1a1a" stroke="#6B728044"/><text x="78" y="138" fill="#6B7280" font-size="8" font-weight="bold" font-family="sans-serif">🔒 Segurança</text></svg>`,

      "config-dados": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="15" width="186" height="130" rx="8" fill="#1a1a1a" stroke="#333"/><text x="72" y="30" fill="#666" font-size="7" font-family="sans-serif">NOME DA BARBEARIA</text><rect x="72" y="34" width="170" height="14" rx="4" fill="#222" stroke="#6B7280"/><text x="80" y="44" fill="#EEE" font-size="7" font-family="sans-serif">Mogiana Barber</text><text x="72" y="60" fill="#666" font-size="7" font-family="sans-serif">TELEFONE</text><rect x="72" y="64" width="80" height="14" rx="4" fill="#222" stroke="#333"/><text x="80" y="74" fill="#EEE" font-size="7" font-family="sans-serif">(16) 3333-3333</text><text x="72" y="90" fill="#666" font-size="7" font-family="sans-serif">ENDEREÇO</text><rect x="72" y="94" width="170" height="14" rx="4" fill="#222" stroke="#333"/><text x="80" y="104" fill="#EEE" font-size="7" font-family="sans-serif">Rua das Palmeiras, 123</text><text x="72" y="120" fill="#666" font-size="7" font-family="sans-serif">INSTAGRAM</text><rect x="72" y="124" width="170" height="14" rx="4" fill="#222" stroke="#333"/><text x="80" y="134" fill="#EEE" font-size="7" font-family="sans-serif">@mogianabarber</text></svg>`,

      "config-asaas": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="15" width="186" height="130" rx="8" fill="#1a1a1a" stroke="#4ADE8044"/><text x="72" y="32" fill="#4ADE80" font-size="9" font-weight="bold" font-family="sans-serif">💳 Pagamentos Online</text><text x="72" y="46" fill="#666" font-size="7" font-family="sans-serif">Status da integração Asaas</text><rect x="72" y="52" width="170" height="18" rx="6" fill="#0d2818" stroke="#4ADE8044"/><text x="90" y="63" fill="#4ADE80" font-size="8" font-weight="bold" font-family="sans-serif">✓ Conta ativa · Sandbox</text><text x="72" y="84" fill="#666" font-size="7" font-family="sans-serif">Aceitar pagamentos via</text><rect x="72" y="90" width="50" height="18" rx="5" fill="#4ADE8022" stroke="#4ADE8044"/><text x="82" y="101" fill="#4ADE80" font-size="8" font-weight="bold" font-family="sans-serif">⚡ Pix</text><rect x="130" y="90" width="50" height="18" rx="5" fill="#60A5FA22" stroke="#60A5FA44"/><text x="136" y="101" fill="#60A5FA" font-size="8" font-weight="bold" font-family="sans-serif">💳 Cartão</text><rect x="188" y="90" width="52" height="18" rx="5" fill="#1a1a1a" stroke="#333"/><text x="196" y="101" fill="#555" font-size="8" font-family="sans-serif">📄 Boleto</text><rect x="72" y="120" width="170" height="18" rx="6" fill="#C9A84C"/><text x="108" y="131" fill="#0A0A0A" font-size="8" font-weight="bold" font-family="sans-serif">Ativar em Produção</text></svg>`,

      "config-whats": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#111"/><rect x="0" y="0" width="56" height="160" fill="#0A0A0A" opacity=".7"/><rect x="64" y="15" width="186" height="130" rx="8" fill="#1a1a1a" stroke="#25D36644"/><text x="72" y="32" fill="#25D366" font-size="9" font-weight="bold" font-family="sans-serif">💬 WhatsApp</text><text x="72" y="48" fill="#666" font-size="7" font-family="sans-serif">NÚMERO DO WHATSAPP</text><rect x="72" y="52" width="170" height="14" rx="4" fill="#222" stroke="#25D366"/><text x="80" y="62" fill="#EEE" font-size="7" font-family="sans-serif">(16) 99272-2728</text><text x="72" y="78" fill="#666" font-size="7" font-family="sans-serif">ENVIAR LEMBRETE</text><rect x="72" y="82" width="80" height="18" rx="5" fill="#25D36622" stroke="#25D36644"/><text x="80" y="93" fill="#25D366" font-size="7" font-weight="bold" font-family="sans-serif">✓ 24h antes</text><rect x="160" y="82" width="80" height="18" rx="5" fill="#25D36622" stroke="#25D36644"/><text x="168" y="93" fill="#25D366" font-size="7" font-weight="bold" font-family="sans-serif">✓ 1h antes</text><text x="72" y="114" fill="#666" font-size="7" font-family="sans-serif">MENSAGEM DE CONFIRMAÇÃO</text><rect x="72" y="118" width="170" height="18" rx="4" fill="#222" stroke="#333"/><text x="80" y="129" fill="#555" font-size="7" font-family="sans-serif">"Olá {nome}! Seu horário foi confirmado..."</text></svg>`,

      "app-download": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#0A0A0A"/><text x="90" y="30" fill="#EEE" font-size="10" font-weight="bold" font-family="sans-serif">Barber Pro App</text><rect x="115" y="40" width="50" height="50" rx="14" fill="#C9A84C22" stroke="#C9A84C44"/><text x="128" y="72" fill="#C9A84C" font-size="24" font-family="sans-serif">✂</text><rect x="40" y="105" width="90" height="40" rx="10" fill="#1a1a1a" stroke="#333"/><text x="56" y="121" fill="#EEE" font-size="8" font-weight="bold" font-family="sans-serif">App Store</text><text x="50" y="133" fill="#555" font-size="7" font-family="sans-serif">Download iOS</text><rect x="150" y="105" width="90" height="40" rx="10" fill="#1a1a1a" stroke="#333"/><text x="160" y="121" fill="#EEE" font-size="8" font-weight="bold" font-family="sans-serif">Play Store</text><text x="156" y="133" fill="#555" font-size="7" font-family="sans-serif">Download Android</text></svg>`,

      "app-login": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#0A0A0A"/><rect x="95" y="5" width="90" height="150" rx="16" fill="#111" stroke="#333"/><text x="120" y="28" fill="#C9A84C" font-size="8" font-weight="bold" font-family="sans-serif">Barber Pro</text><circle cx="140" cy="50" r="16" fill="#C9A84C22" stroke="#C9A84C44"/><text x="132" y="55" fill="#C9A84C" font-size="14" font-family="sans-serif">✂</text><rect x="105" y="78" width="70" height="12" rx="4" fill="#222" stroke="#333"/><text x="113" y="87" fill="#555" font-size="7" font-family="sans-serif">E-mail</text><rect x="105" y="95" width="70" height="12" rx="4" fill="#222" stroke="#333"/><text x="113" y="104" fill="#555" font-size="7" font-family="sans-serif">Senha</text><rect x="110" y="114" width="60" height="12" rx="5" fill="#C9A84C"/><text x="126" y="123" fill="#0A0A0A" font-size="7" font-weight="bold" font-family="sans-serif">Entrar</text><text x="115" y="140" fill="#555" font-size="6" font-family="sans-serif">Entrar com Google</text></svg>`,

      "app-home": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#0A0A0A"/><rect x="90" y="5" width="100" height="150" rx="16" fill="#111" stroke="#333"/><text x="118" y="22" fill="#EEE" font-size="7" font-weight="bold" font-family="sans-serif">Agenda de Hoje</text><rect x="97" y="28" width="86" height="20" rx="4" fill="#1a1a1a" stroke="#C9A84C44"/><rect x="101" y="32" width="30" height="4" rx="1" fill="#EEE"/><rect x="101" y="40" width="50" height="3" rx="1" fill="#555"/><rect x="97" y="52" width="86" height="20" rx="4" fill="#1a1a1a" stroke="#333"/><rect x="101" y="56" width="30" height="4" rx="1" fill="#EEE"/><rect x="97" y="76" width="86" height="20" rx="4" fill="#1a1a1a" stroke="#333"/><rect x="101" y="80" width="30" height="4" rx="1" fill="#EEE"/><rect x="97" y="104" width="40" height="22" rx="6" fill="#1a1a1a" stroke="#C9A84C44"/><text x="103" y="119" fill="#C9A84C" font-size="14" font-family="sans-serif">📅</text><rect x="141" y="104" width="40" height="22" rx="6" fill="#1a1a1a" stroke="#333"/><text x="147" y="119" fill="#555" font-size="14" font-family="sans-serif">👥</text></svg>`,

      "app-notif": `<svg viewBox="0 0 280 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;border-radius:10px"><rect width="280" height="160" fill="#0A0A0A"/><rect x="90" y="5" width="100" height="150" rx="16" fill="#111" stroke="#333"/><text x="110" y="25" fill="#EEE" font-size="7" font-weight="bold" font-family="sans-serif">Notificações</text><rect x="97" y="32" width="86" height="35" rx="6" fill="#0d2818" stroke="#4ADE8044"/><text x="105" y="46" fill="#4ADE80" font-size="7" font-weight="bold" font-family="sans-serif">🔔 Novo agendamento!</text><text x="105" y="56" fill="#555" font-size="6" font-family="sans-serif">João — Corte, 14:00 hoje</text><rect x="97" y="72" width="86" height="35" rx="6" fill="#1a1a1a" stroke="#FBBF2444"/><text x="105" y="86" fill="#FBBF24" font-size="7" font-weight="bold" font-family="sans-serif">⏰ Lembrete</text><text x="105" y="96" fill="#555" font-size="6" font-family="sans-serif">3 atendimentos amanhã</text><rect x="105" y="118" width="70" height="22" rx="8" fill="#4ADE8022" stroke="#4ADE8044"/><text x="118" y="131" fill="#4ADE80" font-size="7" font-weight="bold" font-family="sans-serif">✓ Ativar</text></svg>`,
    };

    const tutorialsHtml = tutorials.map((t, idx) => `
      <div class="tut-card" id="tut-${t.id}" style="background:var(--surface);border:1px solid var(--border);border-radius:16px;overflow:hidden;transition:border-color .2s">
        <div onclick="toggleTutorial('${t.id}')" style="display:flex;align-items:center;gap:14px;padding:18px 20px;cursor:pointer">
          <div style="width:44px;height:44px;border-radius:12px;background:${t.color}18;border:1px solid ${t.color}33;display:flex;align-items:center;justify-content:center;font-size:22px;flex-shrink:0">${t.icon}</div>
          <div style="flex:1">
            <div style="font-size:15px;font-weight:700;color:var(--text)">${t.title}</div>
            <div style="font-size:12px;color:var(--muted);margin-top:2px">${t.steps.length} passos</div>
          </div>
          <div id="tut-arrow-${t.id}" style="color:var(--muted);font-size:18px;transition:transform .2s">›</div>
        </div>
        <div id="tut-body-${t.id}" style="display:none;border-top:1px solid var(--border)">
          <div style="padding:20px">
            <!-- Navegação de passos -->
            <div style="display:flex;gap:6px;margin-bottom:20px" id="tut-steps-${t.id}">
              ${t.steps.map((s, i) => `
                <div onclick="goToStep('${t.id}',${i})" id="tut-step-dot-${t.id}-${i}" style="flex:1;height:4px;border-radius:2px;cursor:pointer;transition:background .2s;background:${i === 0 ? t.color : '#2a2a2a'}"></div>
              `).join("")}
            </div>
            <!-- Conteúdo de cada passo -->
            ${t.steps.map((s, i) => `
              <div id="tut-step-${t.id}-${i}" style="display:${i === 0 ? "grid" : "none"};grid-template-columns:1fr 1fr;gap:16px;align-items:center">
                <div>
                  <div style="display:inline-block;background:${t.color}18;color:${t.color};font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px;margin-bottom:10px">Passo ${i + 1} de ${t.steps.length}</div>
                  <div style="font-size:16px;font-weight:800;color:var(--text);margin-bottom:8px">${s.title}</div>
                  <div style="font-size:14px;color:var(--muted);line-height:1.7">${s.desc}</div>
                </div>
                <div style="border-radius:10px;overflow:hidden;border:1px solid var(--border)">
                  ${mockups[s.mockup] || `<div style="height:160px;background:#1a1a1a;display:flex;align-items:center;justify-content:center;color:var(--muted);font-size:13px">${t.icon}</div>`}
                </div>
              </div>
            `).join("")}
            <!-- Botões de navegação -->
            <div style="display:flex;justify-content:space-between;align-items:center;margin-top:16px">
              <button onclick="prevStep('${t.id}')" id="tut-prev-${t.id}" style="background:var(--surface2);border:1px solid var(--border);color:var(--muted);padding:8px 16px;border-radius:8px;cursor:pointer;font-size:13px;visibility:hidden">← Anterior</button>
              <span id="tut-counter-${t.id}" style="font-size:12px;color:var(--muted)">1 / ${t.steps.length}</span>
              <button onclick="nextStep('${t.id}',${t.steps.length})" id="tut-next-${t.id}" style="background:${t.color};color:#0A0A0A;border:none;padding:8px 16px;border-radius:8px;cursor:pointer;font-size:13px;font-weight:700">Próximo →</button>
            </div>
          </div>
        </div>
      </div>
    ...
    const tutorialColors = Object.fromEntries(tutorials.map(t => [t.id, t.color]));
    const tutorialStepCounts = Object.fromEntries(tutorials.map(t => [t.id, t.steps.length]));
    ...
    const body = `
      <div style="max-width:860px;margin:0 auto">
        ...
        ${tab === "ajuda" ? `
          ...
          <script>
          var tutColors = ${JSON.stringify(tutorialColors)};
          var tutCurrent = {};
          function toggleTutorial(id) {
            var body = document.getElementById('tut-body-' + id);
            var arrow = document.getElementById('tut-arrow-' + id);
            var card = document.getElementById('tut-' + id);
            var open = body.style.display === 'block';
            body.style.display = open ? 'none' : 'block';
            arrow.style.transform = open ? 'rotate(0deg)' : 'rotate(90deg)';
            card.style.borderColor = open ? 'var(--border)' : tutColors[id];
            if (!tutCurrent[id]) tutCurrent[id] = 0;
          }
          function goToStep(id, step) {
            var total = ${JSON.stringify(tutorialStepCounts)}[id];
            var prev = tutCurrent[id] || 0;
            document.getElementById('tut-step-' + id + '-' + prev).style.display = 'none';
            document.getElementById('tut-step-dot-' + id + '-' + prev).style.background = '#2a2a2a';
            document.getElementById('tut-step-' + id + '-' + step).style.display = 'grid';
            document.getElementById('tut-step-dot-' + id + '-' + step).style.background = tutColors[id];
            document.getElementById('tut-counter-' + id).textContent = (step + 1) + ' / ' + total;
            document.getElementById('tut-prev-' + id).style.visibility = step === 0 ? 'hidden' : 'visible';
            var nextBtn = document.getElementById('tut-next-' + id);
            if (step === total - 1) {
              nextBtn.textContent = '✓ Concluído';
              nextBtn.onclick = function() { toggleTutorial(id); };
            } else {
              nextBtn.textContent = 'Próximo →';
              nextBtn.onclick = function() { nextStep(id, total); };
            }
            tutCurrent[id] = step;
          }
          function nextStep(id, total) { var c = tutCurrent[id] || 0; if (c < total - 1) goToStep(id, c + 1); }
          function prevStep(id) { var c = tutCurrent[id] || 0; if (c > 0) goToStep(id, c - 1); }
          </script>
        ` : tab === "assistente" ? `
